...
| 16 — Configurable Portal & Launcher Copy | Designed, not started (2026-07-14) | User asked (4 screenshots) why the Support Portal block has no editor settings, and how to update the hardcoded text across the New Ticket form and its various states — every string in all 7 portal view components plus the floating launcher is hardcoded English, no admin-facing way to change any of it. Design (via brainstorming session, 2026-07-14): full coverage of primary UI copy (not just headline strings) across all 7 portal views and the launcher, split by storage surface — portal text as a sparse block attribute (`copy`, default `{}`) edited via Inspector Controls; launcher text as a new sparse `launcher_copy` setting on a new dedicated "Launcher" Settings tab (moves the existing `launcher_enabled` toggle out of Advanced). One `copy-defaults.js` per surface as the single source of default English text; sparse overrides merge client-side, so an untouched install is byte-identical to today. The block editor canvas gets a state-switchable live-look preview (9 screens) reusing the real view components in a new inert `previewMode` — not a genuinely live app, since the portal makes real mutating REST calls (create tickets, send real customer emails) that an editor previewing the block must never accidentally trigger. Full spec + 13-item checklist in `phase-plan-clickup.md` Phase 16. | 13 checklist items, nothing built yet |
