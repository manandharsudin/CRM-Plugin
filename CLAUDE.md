...
> Last updated: 2026-07-05 (Phase 7 in progress — 7.1–7.6 all fixed and verified (7.3 also fixed a live Phase-6.1 regression); 7.7 (Freemius secret in query string) closed as an accepted risk after researching Freemius's own API docs — no alternative transmission method exists; 7.8 (admin ticket-list sort had no supporting index) fixed and verified — a new composite index plus dropping a CASE-computed sort column that could never be satisfied by any index. 2 findings remain; see §17. Also: Phase 6 complete — multi-product Freemius support fully implemented + visually verified via Playwright; plus a post-Phase-6 Inbox unread-pill bugfix; see §11, §21)
