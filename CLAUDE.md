...
| 10 — Dynamic Inbox Sort | 🔲 Designed, not started (2026-07-07) | User noticed the Inbox's "Sort: Smart" label was static text with no alternate sort ever available. Designed via brainstorming session: 4 sort options (**Smart** default unchanged; **Priority** — pure `priority DESC` ignoring verified/tier; **Newest first** / **Oldest first** — by `last_activity_at`). The static label becomes a real `<select>`, implemented as ordinary React state (unlike the PHP-bridged filter selects, since this control lives entirely inside the React island) and persisted to `localStorage` as a sticky workflow preference. Backend: new whitelisted `sort` route arg on `GET /admin/tickets`, passed to `get_admin_tickets()` as its own parameter (not folded into the `$filters` WHERE-clause array, since it's an ORDER BY concern). Full spec in `phase-plan-clickup.md` Phase 10. | Not yet built — pending implementation plan + user go-ahead |
