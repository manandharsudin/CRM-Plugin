...
> Last updated: 2026-07-05 (Phase 7 opened — 10 Deep QA findings across performance/security/optimization/error-handling, documented not yet fixed; see §17. Also: Phase 6 complete — multi-product Freemius support fully implemented + visually verified via Playwright; plus a post-Phase-6 Inbox unread-pill bugfix; see §11, §21)
...
| 7 — Deep QA Findings | 🔲 Found, not started (2026-07-05) | Full manual code review (all 26 PHP files) across performance/security/optimization/error-handling, requested after Phase 6 shipped. 10 findings, ranked by severity — highlights: stored XSS via WP `display_name` in ticket system messages (7.1), missing index on `wp_stcrm_contacts.email` causing a full table scan on every authenticated portal request (7.2), a synchronous Freemius API call in the public ticket-creation path that violates the plugin's own "never call Freemius synchronously" rule (7.3). Full list + files + fix directions in `phase-plan-clickup.md` Phase 7 (7.1–7.10). **User directive: fix one at a time, do not batch.** | Each finding independently resolved or explicitly deferred with reasoning, same closure pattern as Phase 5 |
