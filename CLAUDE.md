...
| 11 — Inbox Pagination | 🔲 Designed, not started (2026-07-07) | User flagged that only 12 tickets exist locally today but a real install could reach hundreds — the Inbox needs pagination before then. `get_admin_tickets()` already accepts `page`/`per_page` (unused by the frontend until now); designed via brainstorming: numbered "Page X of Y" + total count (user's explicit pick over a no-count Prev/Next heuristic or infinite scroll), page size stays at the existing default of 20. Backend extracts the WHERE-clause-building logic already inside `get_admin_tickets()` into a shared helper reused by both the list query and a new `count_admin_tickets()` — the explicit design goal is that the count can never drift out of sync with whatever filters/search are currently applied, since both queries are built from the same code path. Total exposed via standard `X-WP-Total`/`X-WP-TotalPages` response headers, keeping the response body a bare array per this plugin's existing "no wrapper" convention. Frontend resets to page 1 whenever filters or sort change. Full design in `phase-plan-clickup.md` Phase 11; not yet built. | Not yet built — pending user go-ahead |
