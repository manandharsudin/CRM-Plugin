...
> Last updated: 2026-07-05 (Phase 7 in progress — 7.1 stored XSS, 7.2 missing contacts.email index, 7.3 synchronous Freemius API call (also fixed a second, more severe live bug found in the same function — license-key verification silently broken since Phase 6.1), 7.4 uncached portal-URL lookup duplicate, 7.5 missing agent-alert debounce, and 7.6 unguarded $wpdb->update() calls in STCRM_Freemius_Sync all fixed and verified, 4 findings remain; see §17. Also: Phase 6 complete — multi-product Freemius support fully implemented + visually verified via Playwright; plus a post-Phase-6 Inbox unread-pill bugfix; see §11, §21)
...
| 7 — Deep QA Findings | 🔲 In progress (2026-07-05) — 6 of 10 done | Full manual code review (all 26 PHP files) across performance/security/optimization/error-handling, requested after Phase 6 shipped. 10 findings, ranked by severity. **7.1 (stored XSS via `display_name` in system messages) ✅ fixed + verified.** **7.2 (missing index on `wp_stcrm_contacts.email`, full table scan on every authenticated portal request) ✅ fixed + verified** — also adds a single-product short-circuit skipping the Phase 6.6 sibling-contact lookup entirely. **7.3 (synchronous Freemius API call blocking public ticket creation) ✅ fixed + verified** — license-key verification is now fully async (queued via Action Scheduler, self-retries once on transient failure); also fixed a second, more severe bug found in the same function: `verify_key_via_api()` had been reading the old flat `freemius_api_token` setting Phase 6.1 deleted, meaning license-key ticket verification had been silently broken (always unverified/free, no error surfaced) since Phase 6.1 shipped. **7.4 (4th uncached duplicate of the portal-URL lookup, on the unrate-limited `template_redirect` magic-link path) ✅ fixed + verified** — `STCRM_Auth_Controller::get_portal_url()` now shares the same `stcrm_portal_page_id` transient the other three copies already use. **7.5 (agent-alert emails had no debounce, unlike customer notifications) ✅ fixed + verified** — `queue_agent_alert()`/`handle_agent_alert()` now use the same pending-lock/debounce pattern as the customer-facing reply notification, under independent `stcrm_alert_*` transient keys. **7.6 (`STCRM_Freemius_Sync`'s `$wpdb->update()` calls didn't check return values) ✅ fixed + verified** — all 4 affected methods now log a failure via the existing `log()` helper, confirmed against a real engineered `(product_id, email)` unique-key collision in `debug.log`. 4 findings remain. Full list + files + fix directions in `phase-plan-clickup.md` Phase 7 (7.1–7.10). **User directive: fix one at a time, do not batch.** | Each finding independently resolved or explicitly deferred with reasoning, same closure pattern as Phase 5 |
