...
> Last updated: 2026-07-05 (Phase 7 in progress — 7.1–7.6 all fixed and verified (7.3 also fixed a live Phase-6.1 regression); 7.7 (Freemius secret in query string) closed as an accepted risk after researching Freemius's own API docs; 7.8 (admin ticket-list sort had no supporting index) fixed and verified — a new composite index plus dropping a CASE-computed sort column that could never be satisfied by any index; 7.9 (stale resolved_at after an agent reply) fixed and verified — mirrors the customer-side reopen's existing null-clearing logic. 1 finding remains; see §17. Also: Phase 6 complete — multi-product Freemius support fully implemented + visually verified via Playwright; plus a post-Phase-6 Inbox unread-pill bugfix; see §11, §21)
...
| 7 — Deep QA Findings | 🔲 In progress (2026-07-05) — 9 of 10 done | Full manual code review (all 26 PHP files) across performance/security/optimization/error-handling, requested after Phase 6 shipped. 10 findings, ranked by severity. **7.1–7.6 all ✅ fixed + verified** (stored XSS in system messages; missing `wp_stcrm_contacts.email` index + single-product short-circuit; synchronous Freemius API call moved fully async, which also surfaced and fixed a second live bug — license-key verification silently broken since Phase 6.1 due to a settings-key read that never got updated in that migration; 4th uncached portal-URL-lookup duplicate; missing agent-alert email debounce; unguarded `$wpdb->update()` calls in `STCRM_Freemius_Sync`). **7.7 (Freemius license secret sent as a GET query param) ✅ closed as an accepted risk** — researched Freemius's actual API docs rather than guessing; every license-key endpoint they document uses the same query-string convention, no POST/header alternative exists anywhere; added a code comment explaining this so it isn't mistaken for an unaddressed bug later. **7.8 (admin ticket-list sort had no supporting index) ✅ fixed + verified** — discovered the `priority` ENUM column's declaration order already matches severity order, so the `CASE`-computed sort column (which can never be index-satisfied) was dropped entirely in favor of sorting the raw column directly, paired with a new composite index; confirmed via `EXPLAIN` + `FORCE INDEX` that the index fully eliminates the filesort. **7.9 (`resolved_at` left stale after an agent reply) ✅ fixed + verified** — `STCRM_Admin_Controller::create_message()`'s reply branch now clears `resolved_at`, mirroring the customer-side reopen that already did this correctly. 1 finding remains. Full list + files + fix directions in `phase-plan-clickup.md` Phase 7 (7.1–7.10). **User directive: fix one at a time, do not batch.** | Each finding independently resolved or explicitly deferred with reasoning, same closure pattern as Phase 5 |
