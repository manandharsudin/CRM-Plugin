...
> Last updated: 2026-07-14 (**Phase 16 COMPLETE — Configurable Portal & Launcher Copy.** Built via `superpowers:writing-plans` (16-task plan) + `superpowers:subagent-driven-development` (fresh implementer + dedicated task-scoped reviewer per task, controller-independent commit-hygiene verification before every review). Every portal view component (`NewTicketView.jsx` incl. `SuccessState`, `CapReachedView.jsx`, `AuthView.jsx` incl. `SentState`, `MyTicketsView.jsx`, `EmptyView.jsx`, `ExpiredView.jsx` incl. `CheckInboxState`, `ThreadView.jsx`) now reads merged copy via `useCopy()` (`src/portal/copy-defaults.js`'s `DEFAULT_COPY` + sparse block-attribute overrides) instead of hardcoding text, and gained a `previewMode` prop that replaces real API calls with fixed sample data and no-ops every handler. `blocks/support-portal/block.json` gained a `copy` object attribute (default `{}`); `render.php` localizes it. `editor.jsx` was rewritten from a bare placeholder into a real component: a 9-screen state switcher rendering the real components inertly in the canvas (not a separate mockup, not a genuinely live app — the portal makes real mutating REST calls, so the editor must never trigger them), plus 10 Inspector Controls panels (one per `DEFAULT_COPY` key, including `capReachedPro`, which has its own fields but no dedicated switcher entry). The launcher got the same treatment (`src/launcher/copy-defaults.js`, `Launcher.jsx`'s 5 functions), plus a new `STCRM_Launcher_Copy::DEFAULTS` PHP mirror (used only for Settings-tab placeholder text, since PHP can't import a JS module) and a new dedicated Settings → Launcher tab (moves the existing enable/disable toggle here from Advanced, adds a fully constant-driven loop of 36 text fields across 5 sections). Three real bugs were found and fixed via the review-loop process, not shipped: (1) a `SentState` "use a different email" link that got permanently stuck after a real magic-link send, since same-view `navigate()` calls don't remount a React component with no `key`; (2) a byte-level curly-vs-straight-apostrophe drift between the JS and PHP copy-default mirrors, caught by a full programmatic diff after a visual self-review had falsely claimed a match; (3) a live, reproduced data-loss bug where moving the launcher-enabled checkbox's markup off the Advanced tab left its save-assignment orphaned there, so saving Advanced for any unrelated reason silently reset the launcher off. Final integration pass confirmed against the real site with no mocks: untouched installs render byte-identical to pre-Phase-16; a real block-attribute override and a real Settings override apply simultaneously with zero cross-interference; the block editor's 9-screen preview fires zero `/stcrm/v1/*` requests across the full pass, including active clicking inside the inert canvas. Full spec + task-by-task implementation notes + verification in `phase-plan-clickup.md` Phase 16 and `plans/2026-07-14-phase-16-configurable-copy.md`. See §17.) 2026-07-14 (**Phase 16 designed, not started — Configurable Portal & Launcher Copy.** User asked (4 screenshots) why the Support Portal block has no editor settings at all, and how to update the hardcoded text shown across the New Ticket form and its various states (cap-reached, magic-link-sent, etc.) — every string across all 7 portal view components and the floating launcher is hardcoded English with no admin-facing way to change it. Design (via brainstorming session, 2026-07-14): full coverage of primary UI copy (headings, labels, buttons, static state screens) across all 7 portal views plus the launcher — error/fallback/server-driven text stays hardcoded, deliberately excluded. Storage split by surface: portal text is a new sparse block attribute (`copy`, default `{}`) edited via Inspector Controls; launcher text is a new sparse `launcher_copy` setting on a new dedicated "Launcher" Settings tab (the existing `launcher_enabled` toggle moves here from Advanced). One default-copy source per surface (`copy-defaults.js` × 2) — sparse overrides merge with defaults client-side, so an untouched install renders byte-identical to today, zero migration needed. One accepted trade-off: the launcher's Settings tab is a plain PHP form (unlike the portal's React Inspector Controls), so its default-text placeholders require a small mirrored PHP constants class — low-risk duplication, both files written together in this one phase. Biggest design decision: the block editor's canvas gets a **state-switchable live-look preview** (9 screens: New Ticket, Ticket Submitted, Cap Reached, Sign In, Check Your Inbox, Expired Link, My Tickets, Empty State, Thread) reusing the real view components in a new inert `previewMode` (API calls replaced with fixed sample data, handlers become no-ops) rather than either the old static placeholder or a genuinely live app — the portal isn't a passive display, it makes real mutating REST calls (create a ticket, send a real customer email), so rendering it live in the editor risked an editor accidentally creating real tickets while just previewing the block; `previewMode` reuses the exact same markup/CSS as production, so the preview can't drift out of sync with a separately-maintained mockup. Full spec + 13-item checklist in `phase-plan-clickup.md` Phase 16. See §17. Nothing built yet.) 2026-07-13 (**UI tweak — Settings → Tickets & Guards field grouping + copy clarification, plugin-repo only.** Confirmed (live, ticket #15) that a free-tier ticket correctly locks after 3 follow-up messages, not 3 total including the opener — a deliberate Phase 2 acceptance-testing decision, not a bug. User kept that behavior but asked for the Settings description to say so explicitly, and for `Guard: Silent Ceiling`'s row to sit with the other 3 Guard fields instead of stranded after the Rate Limit group. Copy + row-order only, no behavior change. Full details in the plugin repo's own `CLAUDE.md`.) 2026-07-13 (**Bugfix — ticket-creation rate limits made admin-configurable, plugin-repo only.** User reported "Too many requests" on a fresh-session ticket submission. Root-caused via systematic debugging before any fix — reproduced live against the real REST endpoint, confirming `STCRM_Rate_Limiter::check_ticket_ip()`'s hardcoded 5-per-hour-per-IP cap had simply been exhausted by the day's heavy same-IP testing, not a code defect ("fresh session" is irrelevant to an IP-keyed check). Made the 3 ticket-creation limits (IP/hour, email/day unverified, email/day verified) admin-configurable on Settings → Tickets & Guards instead of leaving them hardcoded, so a busy dev/QA period can raise them without a code change. Verified by raising the IP limit and confirming 7 requests all succeeded (previously would've 429'd at 6), then restoring defaults and confirming the boundary trips again correctly. Per the documentation convention below, no `phase-plan-clickup.md` entry — full details in the plugin repo's own `CLAUDE.md`.) 2026-07-13 (**Phase 15 COMPLETE — Automatic Freemius Entitlement Sync.** Built: `STCRM_Backfill::handle_trigger()`'s reset+enqueue logic extracted into reusable `trigger_for_product( int $product_id )`; new `STCRM_Backfill::sync_all_products()` loops every configured Freemius product, skips non-Freemius/no-token rows and any product already mid-run, triggers the rest. New recurring WP-Cron hook `stcrm_recurring_freemius_sync` on an admin-configurable interval (new Settings → Advanced field `freemius_sync_interval`: hourly/twicedaily/daily/weekly, default daily); a new `SublimeCRM::register_weekly_cron_schedule()` adds `weekly` since it isn't a WP-core built-in. Changing the interval reschedules immediately (starting from `time()`, so the first run fires right away). Activation schedules the default; deactivation/uninstall clear it. No new DB table — confirmed the existing `wp_stcrm_contacts`/`wp_stcrm_contact_products` schema already is the entitlement table this needed. Verified via a PHP-CLI script (13 assertions) plus a real two-process run of the actual Advanced-tab save path confirming the setting persisted and the cron rescheduled correctly. Full spec + verification notes in `phase-plan-clickup.md` Phase 15. See §17.) 2026-07-13 (**Phase 15 designed, not started — Automatic Freemius Entitlement Sync.** User report: a logged-in user could open a ticket, then hit the free-tier "1 open ticket" cap on a third submission, despite this site's configured products all being Pro-tier — traced to backfill never having been run for any of them, so `wp_stcrm_contact_products` had no row to prove real entitlement and every ticket resolved `tier=free`. Investigation confirmed the guard/tier machinery is already correct and identical for logged-in and anonymous submitters — `check_ticket_cap()` keys on `(contact_id, product_id)` regardless of session, no fix needed there. Design (via brainstorming session, 2026-07-13): **no new DB table** — `wp_stcrm_contacts`+`wp_stcrm_contact_products` already is the "product + user email" entitlement table the user proposed building; adding a second one would just create two sources of truth that can drift. The actual fix is turning the existing manual, one-product-at-a-time "Run Backfill" button into one automatic recurring job (`STCRM_Backfill::sync_all_products()`) that covers every configured Freemius product, on an admin-configurable interval (new Settings → Advanced field `freemius_sync_interval`: hourly/twicedaily/daily/weekly, default daily — nothing shorter than hourly, to avoid hammering the Freemius API). The recurring job skips any product already mid-run (`status === 'running'`) so a short interval can't reset progress on a large in-flight paginated import. Changing the interval re-schedules immediately (`wp_clear_scheduled_hook()` + `wp_schedule_event(time(), ...)`) so the very first run fires right away rather than waiting a full interval — closing the reported gap the moment this is built, with zero ongoing manual step after that. The existing per-product webhook handlers (install/upgrade/downgrade/license.expired/cancelled) are unaffected and keep providing real-time updates; this phase is a backstop for historical/missed data, not a replacement. Manual "Run Backfill" button is kept for on-demand/troubleshooting use. Full spec + 7-item checklist in `phase-plan-clickup.md` Phase 15. See §17. Nothing built yet.) 2026-07-12 (**Phase 14 designed, not started — Global Contact Identity Model.** Combines two related bug reports into one design, since both trace back to the same root cause: contacts are scoped per `(product_id, email)`, not per person. Report 1: the free-tier "1 open ticket" cap is really "1 per product," and the same real person shows up multiple times in the admin Contacts list (once per product touched, plus a "shell" `product_id=0` row from any magic-link/WP-SSO sign-in with no prior ticket). Report 2: password login only works for someone who already has a WP account — a brand-new visitor can only ever use magic link first, and the existing "Set a password" on-ramp (Phase 13) only appears after they already have a ticket. Design (via brainstorming session, 2026-07-12): contact identity becomes global — one `wp_stcrm_contacts` row per person, keyed by email, holding only person-level fields (`email`, `name`, `fs_user_id`); everything product-specific (`tier`, `plan`, `license_key_hash`, `license_status`, `license_expires`, `sites_count`, `verification_pending`) moves to a new `wp_stcrm_contact_products` table. The free-tier open-ticket cap explicitly **stays per-product**, not global — only the identity was wrong, not the cap's granularity. Shell contacts (`product_id=0`) are eliminated entirely — a bare contact row with zero `contact_products` rows already means "signed in, no product history yet," so every `upsert_contact(0, 0, ...)` call site simplifies. The password on-ramp doesn't get a new mechanism — magic link stays the only first-time entry point (self-registration was already considered and rejected in Phase 13's own design, for the same reason: a second identity-proof mechanism, its own spam/verification surface) — instead, the existing "Set a password" prompt (today gated to having a ticket) also appears on the very first sign-in with zero tickets. Since this site only holds disposable local test data (confirmed explicitly with the user, not assumed), **no migration code ships** — `install()`'s schema is simply rewritten for future fresh installs, and this local site's 4 tables get reset once, manually, outside any code path that ships in the plugin. Admin Contacts list/detail pages get updated for one-row-per-person (Tier badge = Pro-if-any-product-is-Pro, per-product breakdown table on the detail page, cross-product ticket history). Full spec + 14-item checklist in `phase-plan-clickup.md` Phase 14. See §17. Nothing built yet.) 2026-07-08 (**Bugfix follow-up — portal password login now also starts a real WP session.** Two same-day bugfixes after Phase 13 shipped: (1) portal routing — a bare landing URL forced the New Ticket form even when authenticated, and "View your tickets" dead-ended into the sign-in form; (2) "logged in should mean logged in" — WP login and the portal's own `stcrm_session` were entirely independent by original design, so neither native WP-login recognition nor a correct portal password actually signed a customer in unless a contact already existed; fixed by auto-creating a product-agnostic "shell" contact (`product_id=0`) when none exists, so a proven identity always gets a real session. Then a third report: the portal showed as logged in while WP's own "My Account"/admin bar still showed a guest — because `login_wp_account()` deliberately never called `wp_set_auth_cookie()` (Phase 13's original design: portal sign-in must not double as wp-admin login). Raised the real trade-off directly (this reverses that boundary — the portal password form becomes a second, full wp-admin-equivalent login gate for any WP account) and got explicit confirmation before changing it. Now calls `wp_set_auth_cookie()`/`wp_set_current_user()` on success; `signout()` calls `wp_logout()` symmetrically (confirmed as its own, separate decision). Verified live via Playwright: `/wp-admin/` loads the Dashboard directly post-login (no redirect), portal itself keeps working via WP core's own self-healing nonce retry. Full details in `phase-plan-clickup.md`'s two Bugfix sections after Phase 13.) 2026-07-08 (**Phase 13 COMPLETE — WP Account Login for the Support Portal.** Visitors with an existing plain WP account can now sign in with username/password instead of only email + magic link. New `STCRM_Rate_Limiter::check_wp_login()` (IP + identifier buckets, real 429s since a login endpoint inherently reveals success/failure). New shared `STCRM_Auth_Controller::mint_session()` extracted from `handle_redemption()` — magic-link and the new `POST /stcrm/v1/auth/wp-login` share identical session-cookie logic; `wp_authenticate()` proves identity only, never calls `wp_set_auth_cookie()`. A WP user with no contact yet gets `{has_tickets:false, email}`, no session — frontend opens New Ticket pre-filled/locked. On-ramp: new `POST /stcrm/v1/auth/set-password` (session-authenticated) lets a signed-in customer create a `subscriber` account for next time — 409-guarded against ever touching an existing account. `AuthView.jsx`/`ExpiredView.jsx`/`MyTicketsView.jsx`/`NewTicketView.jsx` all updated. Verified via a PHP-CLI script (24 real-DB assertions), a raw HTTP request confirming the real `Set-Cookie` header, and a Playwright pass. Full spec in `phase-plan-clickup.md` Phase 13. See §17.) 2026-07-08 (**Phase 12 COMPLETE — Free (WP.org) Product Support.** Free themes/plugins hosted only on WordPress.org (no Freemius product) previously had no way to receive support tickets — `resolve_product_id()` hard-rejected anything outside the Freemius-shaped `products` list. Built: every product row gains a `source` field (`freemius` default | `wporg`), still one repeatable list, not a second one; new `STCRM_Settings::is_freemius_product()` is the single branch point reused everywhere (Settings render, ticket creation, webhook loop, backfill trigger, connection-test AJAX) — missing `source` defaults to `freemius`, so every pre-existing row keeps working with zero migration. A `wporg` row needs only a Label; `product_id` auto-assigns from a new `next_wporg_id` counter (900,000,001+, collision-proof against real Freemius IDs), preserved across edits via the same row-key matching `build_products_from_post()` already used for token continuity. Settings "Freemius" tab renamed "Products"; each row's Source `<select>` toggles the Freemius-only fields (Product ID/API Token/Secret Key/Connection Status/Backfill) live via a `data-source` attribute + pure CSS — no duplicated field markup, no extra render path. `resolve_product_id()` (`STCRM_Tickets_Controller`) now returns the full matched row instead of a bare int, so `create_ticket()` branches: Freemius products go through the existing tier resolver unchanged, `wporg` products call a new `STCRM_Tier_Resolver::resolve_free_product()` — always free/unverified, no license key, no API call, no cache side effects. `GET /products` exposes `source` so the ticket form can hide its License Key field for a `wporg` selection. Webhook/backfill/connection-test all explicitly reject or skip `wporg` rows with a clear message. Verified via a PHP-CLI script (26 real-DB assertions: settings save with counter increment + untouched existing credentials, a real `POST /tickets` for a `wporg` product confirming `tier_at_open=free`/`verified=0`/correct default priority, a real webhook signature check still validating against an actual configured Freemius secret) plus a full Playwright browser round trip (live field toggle, a real save persisting a `wporg` row, confirmed correct on reload) — all test data and 3 stray rows from repeated browser-test runs cleaned up afterward. **Before this build started**, re-verified both repos' clean git state against memory and re-checked the design's assumptions against live code, and separately caught + fixed a stale Summary-table row in `phase-plan-clickup.md` (Phase 6 still read "designed, not started" despite being fully built weeks earlier; Phases 1–5 had no completion status shown at all). Full spec + verification notes in `phase-plan-clickup.md` Phase 12. See §17.) 2026-07-08 (**Phase 13 designed, not started — WP Account Login for Portal.** User wants visitors who already have a plain WP account on this site to optionally sign in with username/password instead of a magic-link email, as a convenience alternative (not a replacement). Design (via brainstorming session): embedded login form in `AuthView.jsx` → new rate-limited `POST /stcrm/v1/auth/wp-login` → `wp_authenticate()`. Since `wp_stcrm_tokens.contact_id` is `NOT NULL` and contacts are scoped per `(product_id, email)`, a WP user with no ticket history has nothing to anchor a session to — on no match, no session is minted; the verified email is returned and the frontend opens the New Ticket form pre-filled/locked instead. On a match, mints the identical session cookie the magic-link flow already issues — no changes to any existing session-authenticated route. User then raised the obvious follow-up before this got committed — what about someone with *no* WP account who wants password login? Rather than a full separate self-service registration form (its own feature: open registration, a second email-ownership-proof mechanism, orphaned-account cleanup), My Tickets offers a one-time "Set a password for faster sign-in next time" prompt to anyone already signed in via magic link, creating a brand-new `subscriber`-role account for that email — guarded by `email_exists()` so it can only ever create a new account, never touch an existing one's password. User then asked for an explicit impact review against the existing magic-link flow before committing — found and folded in: session-minting logic (resolve contact → pick anchor → insert token → set cookie) extracted into one shared `mint_session()` helper used by both `handle_redemption()` and the new WP-login handler, so the two paths can't drift; `ticket_id` deep-link context (already threaded through the magic-link request from `AuthView.jsx`) now threads through the password path too; `AuthView.jsx`'s "No password needed" copy flagged for update; `ExpiredView.jsx` gains a cross-link to the password option; `/auth/wp-login` explicitly documented as never calling `wp_set_auth_cookie()` (portal sign-in must not double as a native WP login); auto-created accounts tagged with `stcrm_created` usermeta for admin visibility. Full spec in `phase-plan-clickup.md` Phase 13. See §17. Nothing built yet.) 2026-07-08 (**Phase 12 designed, not started — Free (WP.org) Product Support.** User flagged that free themes/plugins hosted only on WordPress.org have no Freemius product and currently can't receive support tickets at all — `resolve_product_id()` hard-rejects any product not in the Freemius-shaped `products` settings list. Design (via brainstorming session): rename Settings "Freemius" tab to "Products"; free products join the same `products` list via a new `source` field rather than a separate list; free rows need only a Label, with `product_id` auto-assigned from a reserved 900,000,000+ range; free-product tickets always resolve free/unverified with no tier resolution or license-key field. Full spec in `phase-plan-clickup.md` Phase 12. See §17. Nothing built yet.) 2026-07-07 (**Phase 11 COMPLETE — Inbox Pagination.** User flagged that only 12 tickets exist locally today but a real install could reach hundreds. Built numbered "Page X of Y" + total count, page size stays 20. `STCRM_Database::build_admin_tickets_where()` extracted as a shared helper so `get_admin_tickets()` and the new `count_admin_tickets()` build their WHERE clause identically — structurally impossible for the count to drift from the list. Total exposed via `X-WP-Total`/`X-WP-TotalPages` headers, body stays a bare array. Frontend resets to page 1 by calling `setPage(1)` directly in the filter/sort-change handlers (not a separate effect, which would fire one wasted stale-page fetch first). Verified against 8 filter/search combinations (paginated through with per_page=1, zero duplicate/missing IDs) plus a full Playwright round trip against 27 real tickets (15 temporarily inserted, since verified) confirming exact page splits, Prev/Next disabling, and — the explicit correctness bar — a narrowing filter mid-pagination correctly resetting to page 1 and hiding the bar. Full spec + verification in `phase-plan-clickup.md` Phase 11. See §17.) 2026-07-07 (**Phase 10 COMPLETE — Dynamic Inbox Sort.** User noticed "Sort: Smart" was static text; only one sort order ever existed. Built 4 options: Smart (unchanged default), Priority (pure urgency, ignores verified/tier), Newest first, Oldest first — persisted to `localStorage` as a sticky preference, backed by a new whitelisted `sort` route arg on `GET /admin/tickets`. Also fixed a would-be regression found while building: the ticket list's loading state previously unmounted its own header, which would've made the new sort dropdown flicker/reset on every change. Verified at 3 layers: PHP-CLI DB check, PHP-CLI REST-controller check (incl. an invalid value falling back correctly), and a full Playwright browser round trip with real login. Full spec + verification in `phase-plan-clickup.md` Phase 10. See §17.) 2026-07-06 (**Phase 9 COMPLETE — 9.3 Instrument Call Sites.** `STCRM_Logger` calls added across 9 files, 12 action tags. Verified live end-to-end with logging temporarily enabled: a real ticket, admin reply, status change, HMAC-signed webhook, magic-link request, and settings save all produced correct log lines with real Kathmandu timestamps and context. Found and fixed unrelated test-data drift on `default_priority_free`/`default_priority_pro` left over from earlier session testing (not a code defect — `$defaults` were always correct). All test data cleaned up. **Phase 9.2 complete — Settings "Advanced" tab.** 4th Settings tab: `logging_enabled` toggle (off by default) + `delete_on_uninstall` moved verbatim from Tickets & Guards. Verified live: both checkboxes save/reflect correctly, "Uninstall" confirmed gone from Tickets & Guards, and the product list stayed untouched across every save (explicitly re-checked, learning from the 8.1 incident). 9.3 (instrument call sites) not started. **Phase 9.1 complete — Logger Infrastructure.** New `STCRM_Logger` static facade (`info()`/`warning()`/`error()`), off by default, writes Asia/Kathmandu-timestamped lines to a daily plain-text file (`wp-content/uploads/sublime-crm-logs/`, protected by `index.php`+`.htaccess`, 30-day retention cron `stcrm_purge_old_logs`) when enabled. Verified directly: zero disk I/O while disabled, correct file/format/timezone/context when enabled, retention purge removes only files past the 30-day cutoff, cron correctly scheduled/unscheduled on activate/deactivate. 9.2 (Advanced Settings tab) and 9.3 (instrument call sites) not started. Full spec in `phase-plan-clickup.md` Phase 9 (9.1–9.3). See §17. **Phase 8 COMPLETE — both Settings gaps closed.** 8.2 (Connection Status): Settings → Freemius tab now has a per-product "Test Connection" button + cached badge, live-pinging Freemius's `installs.json` with the stored API token only (secret key isn't testable this way — webhook-signature-only); verified against a no-token product, a token-configured product (real outbound call, real HTTP-404 surfaced), and an unconfigured product. Also consolidated the "find product by ID" lookup duplicated in `STCRM_Backfill`/`STCRM_Tier_Resolver` into one `STCRM_Settings::find_product_by_id()`. 8.1 (Default Priority Per Tier, done earlier the same day): `STCRM_Tier_Resolver`'s hardcoded `normal`/`low` priority defaults are now two admin-configurable Settings fields; verified via a live Settings-save → real-ticket-creation round trip (ticket #24, cleaned up). Full spec + verification notes in `phase-plan-clickup.md` Phase 8. See §17. Also: doc cleanup — 7 of Phase 7's findings still showed stale "commit pending" placeholders despite being pushed weeks ago; corrected with real commit hashes. The long-deferred "Scheduled Actions menu missing from WP Tools" note (Phase 1) is also resolved — root cause was WPForms hiding the shared Action Scheduler admin page site-wide, not a SublimeCRM bug; fixed via a `wp-config.php` constant, no plugin code involved. Phase 7 COMPLETE (2026-07-05) — all 10 Deep QA findings resolved: 9 fixed with code changes, 1 (7.7, Freemius secret in query string) closed as an accepted risk after researching Freemius's own API docs. 7.3's fix also surfaced and fixed a live Phase-6.1 regression (license-key verification silently broken since that migration). Every finding verified against the real local install — unit tests, live REST/HTTP round-trips, direct DB reads, and one Playwright pass for the stored-XSS finding. Also: Phase 6 complete — multi-product Freemius support fully implemented + visually verified via Playwright; plus a post-Phase-6 Inbox unread-pill bugfix; see §11, §21)
...
| 12 — Free (WP.org) Product Support | ✅ Complete (2026-07-08) | Free themes/plugins hosted only on WordPress.org (no Freemius product at all) previously had no way to receive support tickets, since `resolve_product_id()` hard-rejected any `product_id` not in the Freemius-shaped `products` settings list. Built: Settings "Freemius" tab renamed "Products"; free products join the *same* `products` list via a new `source` field (`freemius`\|`wporg`) rather than a second list; free rows need only a Label — `product_id` auto-assigned from a new `next_wporg_id` counter (900,000,001+, can't collide with real Freemius IDs), preserved across edits by row-key matching; free-product tickets always resolve `tier=free, verified=0` via a new `STCRM_Tier_Resolver::resolve_free_product()`, skipping tier resolution and the license-key field entirely (no Freemius API involved). New `STCRM_Settings::is_freemius_product()` helper guards every call site (Settings render, ticket validation, webhook match, backfill, connection test) — plain arrays throughout, no new value-object class, matching this plugin's existing style. No DB schema change. Source toggle per row hides/shows the Freemius-only fields via `data-source` + CSS, no JS field duplication. Verified via a PHP-CLI script (26 real-DB assertions incl. a real `POST /tickets` for a wporg product and a real webhook signature check) and a full Playwright browser round trip (live field toggle, real save + reload); all test data cleaned up afterward. Full spec + verification notes in `phase-plan-clickup.md` Phase 12. | All 8 checklist items done — **done** |
| 13 — WP Account Login for Portal | ✅ Complete (2026-07-08) | Visitors who already have a plain WP account on this site (any role, not tied to any specific purchase system) can now sign in with username/password instead of waiting on a magic-link email. Built: embedded login form inside `AuthView.jsx` posting to a new `POST /stcrm/v1/auth/wp-login` (rate-limited via a new `check_wp_login()`, generic error on failure) that calls `wp_authenticate()` — deliberately never `wp_set_auth_cookie()`, so portal sign-in never doubles as a native WP login. A WP user with no ticket history yet gets `{has_tickets:false, email}` with no session minted — the frontend opens New Ticket pre-filled/locked instead, with a real session created the normal way once they actually submit. When a contact does match, mints the *identical* session cookie/token the magic-link flow already issues, via a new shared `mint_session()` helper extracted from `handle_redemption()`. **On-ramp:** My Tickets offers a one-time "Set a password for faster sign-in next time" prompt to anyone already signed in — creates a brand-new `subscriber`-role WP account tagged `stcrm_created` usermeta via a new session-authenticated `POST /stcrm/v1/auth/set-password` (409-guarded against ever touching an existing account's password). `ExpiredView.jsx` gains a cross-link to the password option. No DB schema change. Verified via a PHP-CLI script (24 real-DB assertions incl. both rate-limit buckets, ticket_id ownership resolution, a real new account created and immediately able to log in) plus a raw HTTP request confirming the real `Set-Cookie` header and a Playwright pass confirming both forms render and error correctly. Full spec + verification notes in `phase-plan-clickup.md` Phase 13. | All 11 checklist items done — **done** |
| 14 — Global Contact Identity Model | Designed, not started (2026-07-12) | Combines two related bug reports (Issues.md #3 and #4) into one design: contacts are scoped per `(product_id, email)`, not per person, which causes both the duplicate-Contacts-list-row problem and a per-product (not per-person) open-ticket cap, and forces a `product_id=0` "shell contact" hack for anyone signed in with no product history yet. Design: contact identity becomes global — one `wp_stcrm_contacts` row per person (person-level fields only: `email` now `UNIQUE`, `name`, `fs_user_id`); everything product-specific moves to a new `wp_stcrm_contact_products` table (`tier`, `plan`, `license_key_hash`, `license_status`, `license_expires`, `sites_count`, `verification_pending`, `UNIQUE KEY (contact_id, product_id)`). The open-ticket cap explicitly **stays per-product** — only identity was wrong, not the cap's granularity. Shell contacts are eliminated entirely. Password login's on-ramp doesn't get a new mechanism (self-registration already rejected in Phase 13) — the existing "Set a password" prompt just also appears on the very first sign-in, zero tickets required, instead of only after a ticket exists. No migration code ships — this site's data is disposable local test data (confirmed with the user), so `install()`'s schema is simply rewritten for future installs, and this site's 4 tables get reset once, manually, outside the plugin's own code. Admin Contacts list/detail pages updated for one-row-per-person. Full 14-item checklist in `phase-plan-clickup.md` Phase 14. | 14 checklist items, nothing built yet |
| 15 — Automatic Freemius Entitlement Sync | ✅ Complete (2026-07-13) | User report: a logged-in user hit the free-tier open-ticket cap despite the site only having Pro products configured, because backfill had never been run for any of them — `wp_stcrm_contact_products` had no row to prove the real tier, so every ticket resolved `tier=free`. Investigation confirmed the guard/tier logic itself is correct and already unified across logged-in and anonymous submitters (`check_ticket_cap()` keys on `(contact_id, product_id)` regardless of session) — no new table or session-based branching needed. Built: no new DB table — `wp_stcrm_contacts`+`wp_stcrm_contact_products` already is the "product + user email" entitlement table; the fix turns the existing manual, per-product, one-time "Run Backfill" button into one automatic recurring job (`STCRM_Backfill::sync_all_products()`) covering every configured Freemius product, on an admin-configurable interval (Settings → Advanced: hourly/twicedaily/daily/weekly, default daily — nothing shorter than hourly, to avoid hammering the Freemius API; `weekly` added via a new `cron_schedules` filter since it isn't a WP-core built-in). Skips any product already mid-run (`status === 'running'`) so a short interval can't reset progress on a large in-flight import. Changing the interval re-schedules immediately via `wp_unschedule_event()`+`wp_schedule_event(time(), ...)`, so the first run fires right away rather than waiting a full interval. Manual "Run Backfill" stays for on-demand/troubleshooting use. Verified via a 13-assertion PHP-CLI script plus a real two-process run of the actual Advanced-tab save handler confirming the setting persisted and the cron rescheduled correctly. Full 7-item checklist + verification notes in `phase-plan-clickup.md` Phase 15. | All 7 checklist items done — **done** |
| 16 — Configurable Portal & Launcher Copy | ✅ Complete (2026-07-14) | Every portal view component and the launcher's 5 views now read merged copy (`useCopy()` + sparse overrides) instead of hardcoded text — portal via a new `copy` block attribute edited through Inspector Controls, launcher via a new `launcher_copy` Settings-tab setting. `editor.jsx` rewritten from a bare placeholder into a state-switchable (9 screens) inert preview reusing the real view components' `previewMode` — never a genuinely live app, since the portal makes real mutating REST calls. Built via `superpowers:subagent-driven-development` (16 tasks, fresh implementer + dedicated reviewer per task); 3 real bugs found and fixed via the review loop before shipping: a `SentState` "use another email" link that got permanently stuck (same-view `navigate()` doesn't remount a keyless component), a byte-level apostrophe drift between the JS/PHP copy-default mirrors, and a live data-loss bug where saving the Advanced tab silently reset the launcher's enable/disable toggle. Final integration pass confirmed against the real site: untouched installs render byte-identical to before, overrides on both surfaces apply simultaneously with zero cross-interference, and the block editor's preview fires zero `/stcrm/v1/*` requests across all 9 states. Full spec + task-by-task notes in `phase-plan-clickup.md` Phase 16 and `plans/2026-07-14-phase-16-configurable-copy.md`. | All 13 checklist items done — **done** |

**Documentation convention (2026-07-09):** Bugfixes that aren't part of a tracked phase's scope no longer get written up in `phase-plan-clickup.md`. Log them in the plugin repo's `CLAUDE.md` (Phase Completion section) and this docs repo's `CLAUDE.md` header/changelog only — `phase-plan-clickup.md` stays reserved for phase design + checklist tracking.
