...
- [ ] Rewrite `STCRM_Database::create_contacts_table()` — person-level fields only, `UNIQUE KEY (email)`
- [ ] New `STCRM_Database::create_contact_products_table()` — `contact_id`, `product_id`, `tier`, `plan`, `license_key_hash`, `license_status`, `license_expires`, `sites_count`, `verification_pending`, timestamps; `UNIQUE KEY (contact_id, product_id)`
- [ ] One-time manual reset of this local install's 4 `wp_stcrm_*` tables (contacts, tickets, messages, tokens) — NOT shipped plugin code, run directly once against this dev DB before the rest of this phase can be tested
- [ ] `STCRM_Database`: `get_contact_by_email( string $email )` drops its `product_id` parameter; `get_contacts_by_email()` (Phase 6.6's sibling-lookup helper) deleted — no longer needed, only one contact per email exists
- [ ] New `STCRM_Database` methods for `contact_products`: find-or-create/update a row for `(contact_id, product_id)`
- [ ] `STCRM_Tier_Resolver::resolve()` / `resolve_free_product()` — two-step lookup: find-or-create the global contact by email, then find-or-create/update its `contact_products` row for the ticket's product
- [ ] Every `upsert_contact(0, 0, $email, ...)` call site simplifies to `upsert_contact($email, $name)` — `request_magic_link()`, `login_wp_account()`, `STCRM_Session_Auth::authenticate()`'s SSO fallback
- [ ] `STCRM_Session_Auth::get_contact_ids()` collapses to `[$contact->id]` — no more sibling-contact expansion; every ownership check across `STCRM_Tickets_Controller` (`get_ticket()`, `create_message()`, `resolve_ticket()`, `reopen_ticket()`) simplifies from "in my list of sibling contacts" to "is mine"
- [ ] `check_ticket_cap()` unchanged in shape (still `WHERE contact_id = X AND product_id = Y`) — verify it still enforces correctly once `contact_id` means "the person," not "the person for this product"
- [ ] `SetPasswordPrompt` — currently a local, unexported component inside `MyTicketsView.jsx`; extract to a shared file so `EmptyView.jsx` can render it too, rather than duplicating it
- [ ] `STCRM_Database::get_admin_contacts()` / `count_contacts()` / `get_contacts_last_updated()` rewritten to query the new person-level `contacts` table, `LEFT JOIN contact_products` for tier/plan/license aggregation (Pro-if-any-product-is-Pro) and a product count per contact
- [ ] `STCRM_Database::get_tickets_by_contact()` drops its `product_id` parameter (or makes it an optional filter) — the Contact detail page's ticket history now spans every product, not just one
- [ ] Admin Contacts list page (`render_contacts_page()`): "Product" column → "N products" / single product name; Tier badge = Pro-if-any-product-is-Pro; Plan/License → "—" unless filtered to one product
- [ ] Admin Contact detail page (`render_contact_detail_page()`): flat Tier/Plan/License/Sites block → per-product table (one row per `contact_products` entry); ticket history expands to all products, gains a Product column per row
...
| 14 — Global Contact Identity Model | — | 1 group — designed, not started (2026-07-12) | 14 tasks |
