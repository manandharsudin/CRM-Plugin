...
- [ ] `src/portal/copy-defaults.js` — flat default-copy map for all 9 portal screens (~60-80 keys), single source of truth
- [ ] `src/launcher/copy-defaults.js` — flat default-copy map for the launcher's screens (~15-20 keys)
- [ ] `STCRM_Launcher_Copy::DEFAULTS` (new small PHP class/constant) — mirrors `src/launcher/copy-defaults.js`, used only for Settings-tab placeholder text
- [ ] `blocks/support-portal/block.json` — add `attributes: { copy: { type: 'object', default: {} } }`
- [ ] `blocks/support-portal/render.php` — pass `$attributes['copy']` through `wp_localize_script`'s `stcrmPortal` object (raw, sparse — merge with defaults happens client-side)
- [ ] Every portal view component (`NewTicketView.jsx` incl. its embedded `SuccessState`, `CapReachedView.jsx`, `AuthView.jsx`'s two sub-states, `MyTicketsView.jsx`, `EmptyView.jsx`, `ExpiredView.jsx`, `ThreadView.jsx`) — replace hardcoded JSX text with a merged-copy read (context or prop, avoiding prop-drilling through the view tree); add `previewMode` support (skip API-calling `useEffect`s in favor of fixed sample data, no-op handlers)
- [ ] `src/portal/editor.jsx` — rewritten from the bare placeholder into a real component: state-switcher dropdown, per-screen `InspectorControls` `PanelBody`s (`TextControl`/`TextareaControl`, placeholder = `copy-defaults.js` value), canvas renders the selected view component in `previewMode` with live attribute values
- [ ] `Launcher.jsx` — same merged-copy read pattern as the portal, reading from `window.stcrmLauncher.copy` (localized sparse overrides merged client-side with `src/launcher/copy-defaults.js`)
- [ ] `includes/class-stcrm-launcher.php` — localize `launcher_copy` setting into `window.stcrmLauncher.copy`
- [ ] `STCRM_Settings::$defaults` — add `launcher_copy => []`
- [ ] New "Launcher" Settings tab — moves the existing `launcher_enabled` checkbox here from Advanced; adds grouped text fields for every launcher screen, placeholder text from `STCRM_Launcher_Copy::DEFAULTS`
- [ ] `handle_save()` — new `'launcher'` tab case, sparse-saves only non-empty overridden fields into `launcher_copy`
...
**Process:** given the scope (9 previewable states × ~2 components each, a new editor UI, a new Settings tab) — comparable to Phase 14 in breadth though far more mechanically repetitive than deeply interdependent — implementation plan to be written via `superpowers:writing-plans` once this design is approved, execution strategy (subagent-driven vs. sequential) to be decided there.
...
| 16 — Configurable Portal & Launcher Copy | — | 1 group — Designed, not started (2026-07-14) | 13 tasks |
