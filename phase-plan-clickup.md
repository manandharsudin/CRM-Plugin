...
- [ ] New `STCRM_Rate_Limiter::check_wp_login( string $identifier, string $ip ): true|WP_REST_Response`
- [ ] `STCRM_Auth_Controller`: extract shared `mint_session( int $contact_id, ?int $ticket_id = null ): void` helper from `handle_redemption()`'s existing cookie/token logic
- [ ] New `POST /stcrm/v1/auth/wp-login` route (public) — rate limit → `wp_authenticate()` → generic error on failure (`stcrm_invalid_credentials`); accepts optional `ticket_id`
- [ ] On success: `STCRM_Database::get_contacts_by_email( $user->user_email )` — contacts found → `mint_session()` (verifying `ticket_id` ownership against the resolved contact_ids when present) → `{has_tickets: true, ticket_id?}`; none found → `{has_tickets: false, email}`, no session
- [ ] `AuthView.jsx`: embedded username/password form alongside the existing magic-link request form; forwards `ticket_id` from `getTicketId()`; updates the now-inaccurate "No password needed" copy
- [ ] Frontend routing: `has_tickets: true` → navigate to the returned `ticket_id`'s thread if present, else My Tickets (session cookie already set); `has_tickets: false` → navigate to New Ticket with email pre-filled + locked
- [ ] `ExpiredView.jsx`: add a "Sign in with your password instead →" link to `?view=auth`
- [ ] `GET /stcrm/v1/me` response gains `has_wp_account` (`email_exists( $contact->email )`)
...
- [ ] `MyTicketsView.jsx` (or equivalent account-settings spot): "Set a password for faster sign-in next time" form (password + confirm, basic min-length check) shown when `!has_wp_account`; replaced with "You already have an account — sign in with your password" when `has_wp_account` is true
- Files: `includes/Services/class-stcrm-rate-limiter.php`, `api/class-stcrm-auth-controller.php`, `src/portal/AuthView.jsx`, `src/portal/ExpiredView.jsx`, `src/portal/MyTicketsView.jsx`
...
| 13 — WP Account Login for Portal | — | 1 group — designed, not started (2026-07-08) | 8 tasks |
