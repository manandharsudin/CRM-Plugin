...
- [x] `STCRM_Settings::$defaults['products']` rows gain `source` key (default `'freemius'` on read via `wp_parse_args`); new `next_wporg_id` settings key (default `900000001`)
- [x] New `STCRM_Settings::is_freemius_product( array $row ): bool` helper
- [x] Settings: rename "Freemius" tab label to "Products"; add Source toggle to `render_product_row()` — Freemius branch unchanged (Label/Product ID/API Token/Secret Key/Test Connection/Backfill); wporg branch shows only Label, auto-assigns `product_id` from `next_wporg_id` counter on first save of a new row (preserved on subsequent edits, matched by the existing row-key mechanism `build_products_from_post()` already uses)
- [x] `has_invalid_product_id()` / `has_duplicate_secret()` in `handle_save()` skip `wporg` rows (nothing to validate — no user-typed ID, no secret)
- [x] `GET /stcrm/v1/products` includes `source` per row
- [x] `NewTicketView.jsx` hides the License Key field when the selected product's `source` is `wporg`
- [x] `STCRM_Tickets_Controller::resolve_product_id()` returns the full matched product row (not just an int); `create_ticket()` skips `STCRM_Tier_Resolver::resolve()` for `wporg` products, going straight to `tier=free, verified=0, priority=default_priority_free`
- [x] `STCRM_Webhook`, `STCRM_Backfill`, and the Settings "Test Connection" AJAX handler each guard their product-iteration loop with `is_freemius_product()` (wporg rows have no `secret_key`/`api_token` to match/use — one-line guard each, no restructuring)
- Files: `admin/class-stcrm-settings.php`, `admin/js/stcrm-settings.js`, `admin/css/stcrm-admin.css`, `api/class-stcrm-tickets-controller.php`, `api/class-stcrm-webhook.php`, `includes/Services/class-stcrm-tier-resolver.php`, `includes/Services/class-stcrm-backfill.php`, `src/portal/NewTicketView.jsx`

**Process:** built as one cohesive change (not split into sub-items), per the established one-at-a-time cadence.

**Implementation notes (2026-07-08):**
- `is_freemius_product()` treats a missing `source` key as `'freemius'` — every row saved before this phase keeps working with zero migration routine, matching the design's stated compatibility goal.
- The Source `<select>`/field-toggle needed no JS field-set duplication: the Freemius-only fields live in one `<div class="stcrm-product-freemius-only">` wrapper per row, hidden purely via `.stcrm-product-row[data-source="wporg"] .stcrm-product-freemius-only { display: none; }` in `stcrm-admin.css`; `stcrm-settings.js`'s existing delegated click listener on `#stcrm-products-list` gained one more delegated `change` listener that just writes `row.dataset.source = select.value` — works for both existing rows and JS-cloned new rows with no extra wiring.
- `build_products_from_post()`'s counter parameter is passed **by reference** (`int &$next_wporg_id`) so a single call assigns and increments in one place; `handle_save()` reads the current counter from `self::get_settings()` (already defaulted via `wp_parse_args`) before the call and writes the incremented value back into `$current['next_wporg_id']` after — no separate read-modify-write race, since Settings saves are a single synchronous admin-post request.
- `STCRM_Tier_Resolver::resolve_free_product()` deliberately duplicates only the *shape* of `unverified_result()` (which it calls directly) rather than reimplementing tier logic — it still does a real `STCRM_Database::get_contact_by_email()` lookup so `create_ticket()`'s downstream `contact_id`/rate-limit code path is identical regardless of a ticket's product source.
- **Verified (2026-07-08):** PHP-CLI script — 26 assertions against the real DB and a real REST round trip: settings save via the actual private `build_products_from_post()` (reflection) correctly auto-assigns a wporg id from the counter and increments it by exactly 1, leaves existing Freemius rows' encrypted credentials untouched, `has_invalid_product_id()`/`has_duplicate_secret()` both correctly ignore the new row, `find_product_by_id()` finds it after a real `update_option()` round trip, `GET /products` exposes `source` correctly for both kinds of row, a real `POST /tickets` against the wporg product returns 201 with `verified:false` and the DB row shows `tier_at_open=free`/`verified=0`/the configured `default_priority_free` value/a contact with `tier=free`, and a real webhook HMAC check against an actual configured Freemius secret still validates correctly with the wporg row now present in the list. Full Playwright browser pass logged in as the real admin: confirmed the tab now reads "Products", a new row defaults to the Freemius view, switching Source to wporg live-hides the Freemius-only block, saving persists it with zero error notices, and reloading the page shows the persisted row correctly (label intact, fields still hidden). `npm run build` recompiled `stcrm-portal.js`; `php -l` clean on all 5 touched PHP files; zero SublimeCRM entries in `debug.log` across the whole session. All test tickets/contacts and 3 stray "PLAYWRIGHT-TEST" rows created by repeated browser-test runs were deleted afterward; Settings confirmed restored to exactly the real 3 configured products with the counter reset to 900000001.
- **Before starting this build**, re-verified both repos' git state (clean, matched memory exactly) and Phase 12's own design assumptions against the live code (`resolve_product_id()` still returned a bare int, no `is_freemius_product()` existed yet, both `default_priority_free`/`find_product_by_id()` from Phase 8 were present as expected) — also caught and fixed a stale Summary-table row (Phase 6 was marked "designed, not started" despite being fully built weeks earlier; Phases 1–5 had no completion status shown at all) before this phase's own build began.

---

## PHASE 13 — WordPress Account Login for the Support Portal

> User request (2026-07-08): the portal's only sign-in mechanism today is email + magic link. Some visitors already hold a plain WordPress account on this site (any role, not tied to a specific purchase system) and would rather type a username/password once than wait on an email round trip. Designed via brainstorming session (2026-07-08) — key decisions below.

**Design decisions (confirmed via brainstorming, 2026-07-08):**
- **Scope:** any WP account on the site, not limited to a specific purchase/registration system — a general convenience alternative to magic-link, not a replacement for it. (Note: this site's EDD storefront/customer accounts are separately flagged for removal at cutover per the EDD-removal plan — this feature is deliberately not scoped to that legacy account base.)
- **Entry point:** an embedded username/password form directly inside the portal's `AuthView.jsx`, alongside the existing "Email me a sign-in link" option — user explicitly chose owning a login endpoint in-plugin over redirecting out to `wp-login.php` and back, to keep the whole flow inside the portal UI.
- **Contact resolution / the key wrinkle:** `wp_stcrm_tokens.contact_id` is `NOT NULL` and contacts are scoped per `(product_id, email)`, so a WP user with no prior ticket has no contact row to anchor a session to. Two outcomes after a successful password check, resolved via the existing `STCRM_Database::get_contacts_by_email()` (Phase 6.6): contacts found → mint the identical session cookie/token the magic-link flow already issues, → My Tickets. No contacts found → no session minted yet (nothing to anchor it to); response carries the verified email back to the frontend, which opens the New Ticket form with the email field pre-filled and locked — a real session then gets created the normal way the first time they actually submit a ticket, via the existing `upsert_contact()` path already inside `create_ticket()`. User explicitly chose this "let them open one" outcome over a bare empty-state.
- **Session mechanism:** reuses the exact same `stcrm_session` cookie/token mechanism the magic-link flow already mints (same table, same 30-day expiry, same `STCRM_Session_Auth::authenticate()` check every other portal route already uses) — user explicitly chose this over authenticating every request live against `is_user_logged_in()`, since that would mean two independent auth mechanisms coexisting per-request and a session lifetime tied to the WP login cookie instead of the usual 30 days.
- **Security:** new `STCRM_Rate_Limiter::check_wp_login( string $identifier, string $ip )` (IP + submitted username, transient-based, same shape as `check_ticket_ip`/`check_message`) — unlike magic-link's always-200 anti-enumeration response, a login endpoint inherently reveals success/failure, so this gets a real 429 on abuse instead. Failure responses never distinguish "no such user" from "wrong password" (avoids WP core's own username-vs-password disclosure quirk). No new capability/role requirement — this only proves "you are WP user X," then grants the exact same trust level magic-link already grants for X's own email. `/auth/wp-login` deliberately never calls `wp_set_auth_cookie()` — `wp_authenticate()` here is used purely as an identity check; a successful call issues only the portal's own `stcrm_session` cookie, never a native WP login session. Signing into the support portal must not also sign someone into wp-admin.
- **No DB schema changes.** No changes to `GET /tickets`, `GET /tickets/{id}`, or any other session-authenticated route — they keep trusting `STCRM_Session_Auth::authenticate()` unaware of how the cookie was minted.
- **The on-ramp gap (raised by the user before committing this design):** the above only helps someone who *already* has a WP account. A customer with no WP account who wants password login instead of magic-link has no way to get one. Considered a full self-service "Create an account" form (email+password, no prior magic-link needed) but rejected it here — that's really its own feature (open registration, password rules, a *separate* email-ownership-verification flow, orphaned-account cleanup) and would need its own design pass, not a small add to this phase.
  - **Chosen instead:** magic-link stays the *only* first-time entry point (it already proves email ownership — no second proof-of-ownership mechanism needs to be invented). Once signed in via a magic link, My Tickets shows a one-time **"Set a password for faster sign-in next time"** prompt. Submitting it creates a **brand-new** WP account (`wp_insert_user()`, role `subscriber` — no elevated capabilities) for that email, so `POST /stcrm/v1/auth/wp-login` (above) then works for them going forward. The customer only ever types their **email** to sign in afterward, never a generated username — WP core's `wp_authenticate()` already resolves an `@`-containing login value via `get_user_by('email', ...)` automatically, so the internal `user_login` this creates is never shown to them.
  - **Guard against account takeover:** this must only ever *create* a new account, never touch an existing one's password. If a WP account already exists for the session's email (`email_exists()` truthy), the prompt is replaced with "You already have an account — sign in with your password" instead of offering to (re)set a stranger's password merely because someone triggered a magic link to that address.
  - **Tagging:** accounts created this way get a `stcrm_created = 1` usermeta flag at creation time, so admins can distinguish them from any other WP user later (e.g. in the Users list or a future report) without guessing from role alone.
- **Impact on the existing magic-link flow (found while reviewing this design before commit, 2026-07-08):**
  - The session-minting steps (resolve contact(s) by email → pick an anchor → insert a session token → set the cookie) currently live only inside `handle_redemption()`. `/auth/wp-login` needs the identical steps, so this gets extracted into one shared private helper (e.g. `mint_session( int $contact_id, ?int $ticket_id = null )`) that both `handle_redemption()` and the new WP-login handler call — prevents the two paths' cookie expiry/flags/token-insert logic from drifting apart over time.
  - `AuthView.jsx` already threads a `ticket_id` (via `getTicketId()`) into the magic-link request, so someone arriving at sign-in *from* a specific ticket (e.g. via `ExpiredView` or the `CAP_REACHED` view) lands back on that same thread post-authentication. The password form must carry this too — `ticket_id` (if present) gets threaded into `POST /auth/wp-login`'s request body, and on success (with an ownership check against the resolved contact_ids) the frontend navigates straight to that thread instead of generic My Tickets, matching magic-link's existing behavior exactly.
  - `AuthView.jsx`'s copy — "No password needed — we'll email you a sign-in link." — is no longer accurate once a password option sits next to the magic-link form and needs rewording.
  - `ExpiredView.jsx` (shown on an expired/invalid magic-link token) gains a "Sign in with your password instead →" link to `?view=auth`, for anyone who already went through the set-password flow previously — avoids forcing them through a fresh magic-link wait if they don't need to.
  - `Launcher.jsx` (the floating widget) needs no changes — its "Sign in →" link already just navigates out to the portal's `?view=auth`, so it inherits the new password option for free. Same shared `stcrm_session` cookie applies site-wide regardless of which surface authenticated it.

- [x] New `STCRM_Rate_Limiter::check_wp_login( string $identifier, string $ip ): true|WP_REST_Response`
- [x] `STCRM_Auth_Controller`: extract shared `mint_session( int $contact_id, ?int $ticket_id = null ): void` helper from `handle_redemption()`'s existing cookie/token logic
- [x] New `POST /stcrm/v1/auth/wp-login` route (public) — rate limit → `wp_authenticate()` → generic error on failure (`stcrm_invalid_credentials`); accepts optional `ticket_id`
- [x] On success: `STCRM_Database::get_contacts_by_email( $user->user_email )` — contacts found → `mint_session()` (verifying `ticket_id` ownership against the resolved contact_ids when present) → `{has_tickets: true, ticket_id?}`; none found → `{has_tickets: false, email}`, no session
- [x] `AuthView.jsx`: embedded username/password form alongside the existing magic-link request form; forwards `ticket_id` from `getTicketId()`; updates the now-inaccurate "No password needed" copy
- [x] Frontend routing: `has_tickets: true` → navigate to the returned `ticket_id`'s thread if present, else My Tickets (session cookie already set); `has_tickets: false` → navigate to New Ticket with email pre-filled + locked
- [x] `ExpiredView.jsx`: add a "Sign in with your password instead →" link to `?view=auth`
- [x] `GET /stcrm/v1/me` response gains `has_wp_account` (`email_exists( $contact->email )`)
- [x] New `POST /stcrm/v1/auth/set-password` route (session-authenticated via `STCRM_Session_Auth::authenticate()`) — 409 `stcrm_account_exists` if `email_exists()` is already truthy for the session's email; otherwise `wp_insert_user()` a new `subscriber`-role account (unique username derived from the email local-part, deduped on collision) with the submitted password, tagged with `stcrm_created = 1` usermeta
- [x] `MyTicketsView.jsx` (or equivalent account-settings spot): "Set a password for faster sign-in next time" form (password + confirm, basic min-length check) shown when `!has_wp_account`; replaced with "You already have an account — sign in with your password" when `has_wp_account` is true
- Files: `includes/Services/class-stcrm-rate-limiter.php`, `api/class-stcrm-auth-controller.php`, `src/portal/App.jsx`, `src/portal/AuthView.jsx`, `src/portal/ExpiredView.jsx`, `src/portal/MyTicketsView.jsx`, `src/portal/NewTicketView.jsx`, `src/portal/router.js`, `src/portal/session.js`

**Process:** built as one cohesive change (not split into sub-items), per the established one-at-a-time cadence.

**Implementation notes (2026-07-08):**
- `mint_session()` is a private static method taking `(int $contact_id, ?int $ticket_id)` and returning `bool` (not `void` as originally sketched — the caller needs to know whether the token insert actually succeeded, to decide whether it's safe to treat a session as minted). `handle_redemption()` was refactored to call it instead of duplicating the cookie/token logic inline; both callers now share one code path.
- `login_wp_account()`'s anchor-contact heuristic mirrors `request_magic_link()`'s existing generic path exactly: the most-recently-updated contact match is the default anchor, overridden only when a submitted `ticket_id` resolves to a ticket owned by one of the resolved contact IDs. A `ticket_id` that doesn't belong to any of them is silently ignored (not an error) — the login still succeeds generically against the user's own tickets, matching magic-link's existing anti-enumeration posture for the same scenario.
- `AuthView.jsx`'s `PasswordLoginForm` is a sibling component (not nested inside `AuthView` itself) that receives `setSession` as a normal prop from `AuthView`, which in turn now receives it from `App.jsx` (`RouteView`'s `VIEWS.AUTH` branch) — plain prop-drilling, no globals.
- `NewTicketView.jsx`'s locked-email mechanism reuses the exact same URL-param pattern the router already uses for `ticket`/`view` — a new `getEmailParam()` in `router.js`, read once on mount via `useState(() => ...)` so a stray later URL change doesn't fight the user's typed edits.
- **Verified (2026-07-08):** PHP-CLI script — 24 assertions against the real DB: `check_wp_login()`'s IP bucket (10/hr) and identifier bucket (5/hr) each independently trip and return a real 429; a wrong password returns the generic `stcrm_invalid_credentials` 401 (confirmed indistinguishable from a nonexistent user by code inspection — both paths return the identical error); a correct password against a real existing contact returns `has_tickets:true` and inserts a real `wp_stcrm_tokens` session row; a submitted `ticket_id` genuinely owned by the resolved contact is echoed back as `ticket_id`, one belonging to an unrelated email is silently dropped while the login still succeeds; a real WP user with zero contact rows anywhere returns `{has_tickets:false, email}` with no session row inserted; `GET /me` (called after running the real `authenticate()` permission callback first, matching how WP core actually dispatches it) returns `has_wp_account:true` for an email with a real WP account; `set-password` correctly 409s against an email that already has an account, 422s under 8 characters, and on a valid submission for a brand-new email creates a real `subscriber`-role account tagged `stcrm_created=1` whose password immediately works via a follow-up `wp_authenticate()` call. A raw `curl` POST to the real registered endpoint confirmed the actual `Set-Cookie: stcrm_session=...` header (30-day `Max-Age`, `HttpOnly`, `secure`, `SameSite=Lax`) — and, expectedly, that curl (like a real browser) refuses to persist a `Secure` cookie issued over this site's plain HTTP, so a same-jar follow-up `GET /me` correctly failed with 401 — this is the identical pre-existing constraint already documented for magic-link's own session cookie on this local install, not a Phase 13 regression; the DB-level assertions above are what actually prove the session-minting logic is correct. A full Playwright pass on the live portal confirmed both the password and magic-link forms render, a wrong-password submit shows the correct generic error text, a correct-password submit throws zero JS errors, and `ExpiredView`'s new cross-link renders and is clickable. `npm run build` recompiled `stcrm-portal.js`; `php -l` clean on both touched PHP files; zero real plugin errors in `debug.log` (only pre-existing unrelated EDD deprecation notices and CLI-only "headers already sent" noise from running `setcookie()` under the PHP CLI SAPI, which doesn't occur in real requests — confirmed separately via the `curl` check above). All test WP users/contacts/tickets/tokens created during verification were deleted afterward; a final sweep confirmed zero leftover `phase13`-tagged rows anywhere.

---

### Bugfix — Portal routing + "logged in should mean logged in" (2026-07-08, found after Phase 13)

User report #1: "When I'm logged in, viewing a ticket shows the login form instead of active tickets, and if no ticket has been created yet it also shouldn't show the login form." Root-cause investigated (not the WP-nonce theory initially suspected — that was a real but separate simplification, see below): `router.js`'s `getView()` defaulted a bare `?view=`-less landing URL to the *literal* string `'new-ticket'`, making it indistinguishable from an explicit `?view=new-ticket` (which the "+ New ticket" buttons rely on to force the form even for an authenticated customer). `App.jsx`'s `RouteView` special-cases that exact name to always render `NewTicketView`, so an authenticated customer's very first, bare-URL page load always hit that branch instead of the auth-aware fallthrough already written below it (My Tickets / EmptyView) — contradicting the file's own documented routing table. Fixed: `getView()` now returns `null` when the param is absent; `RouteView`'s existing fallthrough logic (previously unreachable for a bare landing) now runs unmodified. `NewTicketView.jsx` also had two "View your tickets" links that unconditionally `navigate(VIEWS.AUTH)` regardless of session state — now routes to `MY_TICKETS` when already authenticated. Verified live (wp-cli-minted session cookie + Playwright): authenticated + zero tickets + bare URL → empty-tickets state, not login; authenticated + one ticket + bare URL → shows that ticket; "View your tickets" while authenticated → My Tickets, never the login form.

While investigating, also found and fixed a real but unrelated hardening gap: the portal's own `src/portal/api.js` and `src/launcher/index.js` registered a redundant WP REST nonce middleware neither surface needs (every route is `stcrm_session`-cookie authenticated, never `current_user_can()`-gated) — removed, since a stale nonce (e.g. the browser's separate WP-admin login session rotating while the SPA tab stays open) could spuriously 403 a perfectly valid customer session via WP core's own `rest_cookie_check_errors()`. Also fixed `AuthView.jsx`'s `PasswordLoginForm`, which navigated into an authenticated view after login without checking that the post-login session refresh actually confirmed authentication first.

**User report #2 (deeper, after the above fix shipped):** "logged in with WordPress but the portal still shows the login form" — traced to the portal session being, by original design, entirely independent of WP login: `STCRM_Session_Auth::authenticate()` only ever checked the `stcrm_session` cookie, never `is_user_logged_in()`. Per explicit user decision (asked directly rather than assumed): a logged-in WP user should now be auto-recognized by the portal, matching an existing contact by email if one exists.

**User report #3 (same investigation, one layer deeper):** password login via `POST /auth/wp-login` for a WP account with **no existing ticket** correctly returned `{has_tickets:false}` and routed to the New Ticket form (by Phase 13's original design — contacts are scoped per `(product_id, email)`, created only via a ticket, so there was nothing to anchor a session to). User's explicit correction: *"what is the confusion here? The issue is when user logs in with user and pass, it should be logged in, that's it."* — i.e. this was never meant to be conditional on ticket history.

**Fix, applied consistently to both the native WP-login SSO path and the password-login form:** a proven WP identity (native login-cookie recognition, or a correct password via the form) with no existing contact now gets a product-agnostic **"shell" contact** created (`STCRM_Database::upsert_contact( 0, 0, $email, $name, 'free' )` — `product_id = 0`, a value no real Freemius or wporg product ID can ever collide with, per the existing `UNIQUE KEY (product_id, email)`), then a real session is minted against it exactly as if a contact had always existed. `login_wp_account()`'s response field was renamed `has_tickets` → `authenticated` (now always `true` on success, since there's no more "no session" branch). The real, product-scoped contact row still gets created separately by `upsert_contact()` the moment they actually file their first ticket — the shell row is untouched by that and just continues representing their zero-ticket account state, resolved alongside any real contact via the existing Phase 6.6 sibling-lookup by email.

**Also fixed, surfaced while reproducing report #2/#3 (real, not environment-specific):**
- `session.js`'s `detectSession()` fired its `/tickets` and `/me` probes sequentially (`await` one after the other); switched to concurrent (`Promise.allSettled`) — this local install's REST round trips run ~1–2s each (confirmed via `curl` against unrelated, unauthenticated endpoints, e.g. bare `/wp-json/` — an environment characteristic, not something this session caused), so serial calls meant a multi-second wait after every login with only a static "Signing in…" button as feedback; now roughly halved.
- `App.jsx` had no protection against two independent `detectSession()` calls resolving out of order — the page's own mount-time probe, and the one `AuthView.jsx` fires right after a login, could complete in either order, and whichever settled *last* silently won regardless of which was actually current; a slow, stale "not authenticated" mount-probe response landing after a fresh, correct post-login "authenticated" response would revert a successful login straight back to logged-out. Fixed with a monotonic `sessionVersion` ref: a `detectSession()` continuation is only applied if no newer session update has landed since it started.

**Verification (2026-07-08):** every fix in this bugfix note was checked against the real, live site (not mocks) via Playwright + wp-cli, repeated multiple times from a freshly-cleaned zero-contact state each run: bare-landing routing (both zero-ticket and one-ticket cases), "View your tickets" while authenticated, native WP-login SSO auto-signing in a contact-less account, password login auto-signing in a contact-less account (3+ consecutive real runs, each starting from a deleted contact, all landing on My Tickets with a real `stcrm_session` cookie set), and the parallelized `detectSession()`'s timing improvement (full round trip measured via response timestamps). Confirmed both over `http://` and `https://` — the cookie's unconditional `Secure` flag means it can never persist over plain HTTP in a real browser, a pre-existing, correct constraint (already documented for magic-link) and not something to relax. All test contacts/tokens and rate-limiter transients created during verification were deleted afterward.
...
## PHASE 14 — Global Contact Identity Model

> Combines two related user reports (2026-07-12) into one design, since both trace back to the same root cause: contacts are scoped per `(product_id, email)`, not per person. **Issue (Issues.md #4):** "Guard: Free open Tickets setting is not correctly implemented. Is it due to duplicate contact with same email? Currently there are seperate contact entry for same email, why?" — confirmed live: the same real person shows up multiple times in the admin Contacts list (once per product touched, plus a "shell" `product_id=0` row from any magic-link/WP-SSO sign-in with no prior ticket), and the free-tier "1 open ticket" cap is really "1 per product," not the true per-person limit the Settings field's own description implies. **Issue (Issues.md #3):** "What if user want to login with user/pass instead of magic link who isn't WP user yet?" — today, password login only works for someone who already has a WP account; a brand-new visitor can only ever use magic link first, and the existing "Set a password" on-ramp (Phase 13) only appears after they already have a ticket. Both issues share the same underlying mechanism (product-scoped contact identity, and the `product_id=0` shell-contact hack it forces), so they're designed together. Designed via brainstorming session (2026-07-12).

**Design decisions (confirmed via brainstorming, 2026-07-12):**
- **Contact identity becomes global, not per-product.** A real person gets exactly one `wp_stcrm_contacts` row regardless of how many products they touch — fixes the duplicate-Contacts-list-row problem and gives tickets/sessions/tokens one unambiguous owner. `wp_stcrm_contacts` is rewritten down to person-level fields only (`id`, `email` — now `UNIQUE`, `name`, `fs_user_id`, `created_at`, `updated_at`); everything product-specific (`tier`, `plan`, `license_key_hash`, `license_status`, `license_expires`, `sites_count`, `verification_pending`) moves to a new `wp_stcrm_contact_products` table, `UNIQUE KEY (contact_id, product_id)`. `fs_user_id` moves onto the contact row (not per-product) — it's a Freemius identifier for the *person*, shared across every product they've bought from this vendor.
- **The free-tier open-ticket cap stays per-product, not global**, explicitly confirmed — a customer using two different products can still have one open ticket on each simultaneously; only the underlying identity was wrong, not the cap's granularity. `check_ticket_cap()`'s query is unchanged in shape (`WHERE contact_id = X AND product_id = Y`), `contact_id` just now unambiguously means "this person" instead of "this person for this product."
- **Shell contacts (`product_id=0`) are eliminated entirely** — with contacts no longer product-scoped, a bare contact row with zero `contact_products` rows already correctly represents "signed in, no product history yet." Every `upsert_contact(0, 0, $email, ...)` call site (in `request_magic_link()`'s zero-history branch, `login_wp_account()`'s zero-contact branch, the WP-SSO fallback in `STCRM_Session_Auth::authenticate()`) simplifies to a plain `upsert_contact($email, $name)`.
- **Issue #3's password on-ramp moves earlier, not to a new mechanism.** Magic link stays the sole first-time entry point (already proves email ownership — explicitly not reopening the "should we build self-registration" question already rejected in Phase 13's own design for the same reason: a second identity-proof mechanism, its own verification/spam surface). What changes: the existing "Set a password for faster sign-in next time" prompt (today gated to `MyTicketsView.jsx`, i.e. only shown once a ticket exists) also gets added to `EmptyView.jsx`, so it's offered immediately after the *first* successful magic-link sign-in, zero tickets required — directly answering "isn't a WP user yet" by getting them to one in the very first session instead of waiting for a second visit.
- **No migration code ships.** This site's SublimeCRM data is disposable local test data, not production — confirmed explicitly with the user rather than assumed. `install()`'s table definitions are simply rewritten to the new schema (correct for any *future* fresh install); this local site's 4 `wp_stcrm_*` tables get dropped and recreated once, manually, outside of any code path that ships in the plugin — so no future real customer's data is ever at risk from an automatic destructive migration hiding in an upgrade path.
- **Admin Contacts pages get updated for one-row-per-person:** the list page's "Product" column becomes "N products" (or the single product name, if only one); the Tier badge shows Pro if *any* of the person's products is Pro, Free otherwise; Plan/License columns show "—" unless the Product filter is narrowed to exactly one product. The detail page's flat Tier/Plan/License/Sites block becomes a small per-product table (one row per `contact_products` entry); ticket history expands from "this one product's tickets" to all of that person's tickets across every product, with a Product column added to each row.

- [x] Rewrite `STCRM_Database::create_contacts_table()` — person-level fields only, `UNIQUE KEY (email)`
- [x] New `STCRM_Database::create_contact_products_table()` — `contact_id`, `product_id`, `tier`, `plan`, `license_key_hash`, `license_status`, `license_expires`, `sites_count`, `verification_pending`, timestamps; `UNIQUE KEY (contact_id, product_id)`
- [x] One-time manual reset of this local install's 4 `wp_stcrm_*` tables (contacts, tickets, messages, tokens) — NOT shipped plugin code, run directly once against this dev DB before the rest of this phase can be tested
- [x] `STCRM_Database`: `get_contact_by_email( string $email )` drops its `product_id` parameter; `get_contacts_by_email()` (Phase 6.6's sibling-lookup helper) deleted — no longer needed, only one contact per email exists
- [x] New `STCRM_Database` methods for `contact_products`: find-or-create/update a row for `(contact_id, product_id)`
- [x] `STCRM_Tier_Resolver::resolve()` / `resolve_free_product()` — two-step lookup: find-or-create the global contact by email, then find-or-create/update its `contact_products` row for the ticket's product
- [x] Every `upsert_contact(0, 0, $email, ...)` call site simplifies to `upsert_contact($email, $name)` — `request_magic_link()`, `login_wp_account()`, `STCRM_Session_Auth::authenticate()`'s SSO fallback
- [x] `STCRM_Session_Auth::get_contact_ids()` collapses to `[$contact->id]` — no more sibling-contact expansion; every ownership check across `STCRM_Tickets_Controller` (`get_ticket()`, `create_message()`, `resolve_ticket()`, `reopen_ticket()`) simplifies from "in my list of sibling contacts" to "is mine"
- [x] `check_ticket_cap()` unchanged in shape (still `WHERE contact_id = X AND product_id = Y`) — verify it still enforces correctly once `contact_id` means "the person," not "the person for this product"
- [x] `SetPasswordPrompt` — currently a local, unexported component inside `MyTicketsView.jsx`; extract to a shared file so `EmptyView.jsx` can render it too, rather than duplicating it
- [x] `STCRM_Database::get_admin_contacts()` / `count_contacts()` / `get_contacts_last_updated()` rewritten to query the new person-level `contacts` table, `LEFT JOIN contact_products` for tier/plan/license aggregation (Pro-if-any-product-is-Pro) and a product count per contact
- [x] `STCRM_Database::get_tickets_by_contact()` drops its `product_id` parameter (or makes it an optional filter) — the Contact detail page's ticket history now spans every product, not just one
- [x] Admin Contacts list page (`render_contacts_page()`): "Product" column → "N products" / single product name; Tier badge = Pro-if-any-product-is-Pro; Plan/License → "—" unless filtered to one product
- [x] Admin Contact detail page (`render_contact_detail_page()`): flat Tier/Plan/License/Sites block → per-product table (one row per `contact_products` entry); ticket history expands to all products, gains a Product column per row
- Files: `includes/Database/class-stcrm-database.php`, `api/class-stcrm-tickets-controller.php`, `api/class-stcrm-auth-controller.php`, `api/class-stcrm-session-auth.php`, `includes/Services/class-stcrm-tier-resolver.php`, `includes/Services/class-stcrm-guard-matrix.php` (verify only, no expected change), `admin/class-stcrm-admin.php`, `src/portal/EmptyView.jsx`, `src/portal/MyTicketsView.jsx`

**Process:** built as one cohesive change (not split into sub-items) — the schema rewrite and its consumers are too interdependent to build incrementally; verification happens once at the end against the full flow (multi-product ticket creation, guard cap, all 3 auth paths, both admin Contacts pages), same cadence as Phase 12/13.

**Implementation notes (2026-07-13):**
- Built via `superpowers:subagent-driven-development` (fresh implementer + independent reviewer subagent per task, ledger-tracked) — chosen over inline execution, direct on `master` matching every prior phase's convention rather than an isolated worktree.
- **Task 1** — an informal "Phase 14" code-comment label from an unrelated 2026-07-08 bugfix (the WP-login SSO / shell-contact auto-login note above) predated this real, numbered Phase 14; renamed across 6 files before any functional work began, to avoid two unrelated things both being called "Phase 14" in the codebase.
- **Tasks 2–3** — schema rewrite + the local site's one-time manual table reset (controller-run directly, not shipped code) landed cleanly; confirmed post-reset shape via `SHOW COLUMNS`: `contacts` = 6 columns, `contact_products` = 12 columns, `STCRM_DB_VERSION` bumped to `1.8.0`.
- **Task 4** — full DB access layer rewrite (`upsert_contact()`, new `upsert_contact_product()`/`get_contact_product()`, `get_contact_by_email()` dropped its `product_id` param, `get_contacts_by_email()` deleted, `get_contact_by_license_key_hash()` rejoined against `contact_products`, `get_admin_contacts()`/`count_contacts()`/`get_tickets_by_contact()` all rewritten for one-row-per-person).
- **Task 5** — tier resolver's two-step lookup (find-or-create the global contact, then find-or-create/read its `contact_products` row for the ticket's product); `verified_result()`/`unverified_result()` gained a `$cp` parameter, return shape to callers unchanged.
- **Task 6 (scope grown during planning)** — the Freemius webhook handler and backfill sync both wrote tier/plan/license fields directly onto `contacts`, a scope gap found while writing the implementation plan (not in the original design doc) and folded in as not-optional, since those columns no longer exist there: `handle_install_event()`, `handle_user_updated()`, `handle_license_status_change()`, `STCRM_Backfill::process_single_install()` all rewritten to the two-step contact/contact_products upsert.
- **Task 6b (ad-hoc, found during Task 6's review)** — two more webhook handlers in the same file, `handle_plan_changed()`/`handle_license_expiry_changed()`, were outside Task 6's listed scope but still called the old two-argument `get_contact_by_email( $product_id, $email )` against the new one-argument signature (PHP silently discards the extra argument rather than erroring, so `$product_id` was being used as the email lookup value) and wrote directly to `contacts.plan`/`contacts.license_expires` — columns the schema rewrite had already removed. Fixed to match the preserve-untouched-fields pattern already established in `handle_license_status_change()`.
- **Task 7** — shell contacts (`product_id=0` sentinel) eliminated across `request_magic_link()`'s zero-history branch, `login_wp_account()`, and `STCRM_Session_Auth::authenticate()`'s SSO fallback — all three collapse to a plain unconditional `upsert_contact( $email[, $name] )`, since there's only ever one contact per email now. `get_contact_ids()` collapses to a single-element array (kept as an array so every existing `in_array()` ownership check across `STCRM_Tickets_Controller` keeps working unmodified). Review found one in-scope leftover (a stale, self-contradicting comment block in `request_magic_link()` describing the old multi-contact/sibling-anchor behavior this task had just removed) — fixed directly by the controller, a 7-line deletion, no subagent round-trip needed.
- **Task 7b (ad-hoc, severe — found during Task 7's review)** — `STCRM_Session_Auth::authenticate()`'s cookie-token SQL still selected `c.tier`, `c.plan`, `c.license_status`, `c.license_expires`, `c.sites_count` from `wp_stcrm_contacts`, all five removed by the schema rewrite. This broke the query with a DB error on **every** cookie-session-authenticated request — the entire returning-visitor portal auth path, not an edge case. Fixed by trimming the query/`$contact` construction to only the columns still on `contacts`. This also exposed a second, related fault: `get_ticket()`/`create_message()` in `STCRM_Tickets_Controller` read a `$ticket_owner->tier ?? $contact->tier` fallback that was already dead post-schema-rewrite (both operands unset) and would have thrown a fatal `TypeError` into `compute_composer()`'s non-nullable `string $tier` parameter, or silently broken rate-limit/turn-limit gating in `create_message()`, the moment the SQL fix removed `->tier` from `$contact` too. Both now resolve tier fresh via `STCRM_Database::get_contact_product( $contact_id, $ticket->product_id )`, since tier is per-product now, not a contact-level field.
- **Task 8** — verification-only (no code change): confirmed the per-product open-ticket cap still holds exactly as designed once `contact_id` means "the person" rather than "the person for this product," against 3 real configured products.
- **Task 9** — admin Contacts list/detail pages rewritten for one-row-per-person: `product_count`/`has_pro` aggregation and an "N products" column on the list, a per-product table on the detail page, ticket history spanning every product with a new Product column per row. New `resolve_product_label_for_contact()`/`single_product_field()` helpers (the latter whitelist-guarded against SQL injection, even though every call site only ever passes a literal string today).
- **Task 10** — `SetPasswordPrompt` extracted out of `MyTicketsView.jsx` into its own shared `src/portal/SetPasswordPrompt.jsx` so `EmptyView.jsx` renders it too — the password on-ramp now appears on the very first sign-in (zero tickets), not only once a ticket exists. Directly closes Issues.md #3.
- **Task 11b (ad-hoc, found while running Task 11's final combined scenario)** — `get_admin_contacts()`/`count_contacts()` computed `product_count` and the per-product admin filter entirely from `contact_products` rows. Since a `contact_products` row is only ever created via a *verified* resolution path (an active license key), a free-tier ticket submitted with no license key — the common case for any product with a free tier — never creates one, by design. This made free-tier-only contacts show `product_count = 0` and vanish entirely when an admin filtered the Contacts page to a specific product, and undercounted that filter's "N contacts" total. Both methods now recognize product engagement from EITHER `contact_products` OR `tickets` (a `UNION` for the count, `OR EXISTS` for the filter) — `has_pro`/`product_updated_at` deliberately left `contact_products`-only, since a bare ticket can't substantiate a pro-tier claim. Flagged to the user directly before fixing, since it changed what "touching a product" means for an admin-facing view — user chose to fix it within this same phase rather than defer it.

**Verified (2026-07-13):** every task verified individually at the time it landed (disposable PHP-CLI scripts round-tripping the real DB — `verify_task4_db_layer.php` through `verify_task11b_contacts_filter.php` — plus real Playwright passes for Task 9's admin Contacts pages and Task 10's password on-ramp UI, the latter confirming a real new WP account created end-to-end). At the end of the phase, every prior task's PHP-CLI script was re-run back-to-back against the fully assembled codebase (not just individually) to catch any cross-task interaction a single-task run might have missed — all matched their original individual-run results exactly. A final combined multi-product, multi-auth-path scenario then exercised the whole phase's design goal end-to-end in one pass: a person created via magic link (zero tickets, confirmed `has_wp_account:false` so `SetPasswordPrompt` would show) → a free-tier ticket filed on Product A → an active-license ticket filed on Product B (pro tier on B only) → confirmed exactly **one** `wp_stcrm_contacts` row for the email throughout → confirmed the guard cap correctly blocked a 2nd Product A ticket (free cap = 1) while allowing a 2nd Product B ticket (pro cap = 5, not yet hit) → full cleanup confirmed zero residual rows across all 5 SublimeCRM tables. All 10 checkpoints passed. Final `php -l` clean across all 9 touched PHP files (`class-stcrm-database.php`, `class-stcrm-tier-resolver.php`, `class-stcrm-freemius-sync.php`, `class-stcrm-backfill.php`, `class-stcrm-tickets-controller.php`, `class-stcrm-auth-controller.php`, `class-stcrm-session-auth.php`, `class-stcrm-admin.php`, `sublime-crm.php`); final `npm run build` clean with zero working-tree diff (matching Task 10's already-committed build output exactly). 11 planned tasks plus 4 ad-hoc follow-ups (Task 6's scope growth, Task 6b, Task 7b, Task 11b) all independently reviewed by a fresh subagent before being marked complete — one review (Task 7) came back NEEDS FIXES for a single stale comment, fixed inline and not re-dispatched given the triviality; every other review came back APPROVED on first or (for Task 5, after one unrelated session-limit API failure) second dispatch.

---

## PHASE 15 — Automatic Freemius Entitlement Sync

> User report (2026-07-13): a logged-in user could open more than one ticket, then hit the free-tier cap on a third — traced to this dev site having only Pro-tier Freemius products configured with backfill never run for any of them, so every ticket resolved `tier=free` (no `contact_products` row existed to prove otherwise). Investigation confirmed the guard/tier machinery itself is correct: `check_ticket_cap()` already applies `guard_free_open`/`guard_pro_open` identically for logged-in and anonymous submitters, keyed by `(contact_id, product_id)` — there is no session-based branching to fix. The real gap is that `wp_stcrm_contact_products` (the "product + user email" entitlement table) only gets populated by a webhook event, a submitted license key's async re-check, or an admin manually clicking "Run Backfill" once per product — nothing ever prompts that manual step, so a newly-configured or long-forgotten product can sit with zero synced contacts indefinitely. Designed via brainstorming session (2026-07-13).

**Design decisions (confirmed via brainstorming, 2026-07-13):**
- **No new DB table.** `wp_stcrm_contacts` (email) + `wp_stcrm_contact_products` (product_id, tier, license_status, plan, expires, ...) already is the "table of products and their respective user email from Freemius" this phase needed — confirmed with the user rather than adding a duplicate table, which would create two entitlement sources that can drift out of sync. `STCRM_Tier_Resolver::resolve()`'s existing lookup already does "check this table by email/product to decide free/pro" — untouched by this phase.
- **Anonymous and logged-in submitters use one unified rule, confirmed as correct and left unchanged**: `guard_free_open`/`guard_pro_open` apply per `(contact_id, product_id)` regardless of session state — no new anonymous-specific restriction was wanted or added.
- **The fix is entirely about how `contact_products` gets populated**: turn the existing manual, per-product, one-time "Run Backfill" button into one automatic recurring job that covers every configured Freemius product, on an admin-configurable interval — closing the gap where a product is added (or was added before this phase) and nobody remembers to sync it. Ongoing freshness after that first sync is already handled by the existing webhook handlers (install/upgrade/downgrade/license.expired/cancelled) — this phase is a backstop for historical/missed data, not a webhook replacement.
- **Overlap safety**: the recurring job must skip any product whose backfill is already `status === 'running'` — otherwise a sync interval shorter than one full paginated pass (large install counts) would reset progress mid-run and could never converge.
- **Interval is admin-configurable** (Settings → Advanced), using WP's built-in cron schedules (`hourly`/`twicedaily`/`daily`/`weekly`, default `daily`) rather than a custom interval — deliberately not offering anything shorter than hourly, to avoid hammering the Freemius API. Changing the setting re-schedules immediately (`wp_clear_scheduled_hook()` + `wp_schedule_event(time(), ...)`), starting from `time()` so the very first run fires right away instead of waiting a full interval — this alone closes the exact gap that caused the reported bug, with no extra manual step required going forward.
- **Manual "Run Backfill" button is kept**, for on-demand/troubleshooting use — no longer required for routine operation once the recurring job is scheduled.
- **Every Freemius purchaser becomes a contact row, even ones who've never opened a ticket** — already true of today's manual backfill (`process_single_install()` unconditionally upserts a contact per install); this phase just makes it continuous. Confirmed acceptable with the user, since it's the explicit goal of this phase, not a new side effect.

- [ ] Extract `STCRM_Backfill::trigger_for_product( int $product_id ): void` from `handle_trigger()`'s reset+enqueue logic (reset `OPTION_KEY_PREFIX`/`TOTAL_KEY_PREFIX`, set `STATUS_KEY_PREFIX` to `'running'`, `as_enqueue_async_action('stcrm_run_backfill_page', [1, product_id], 'stcrm')`); `handle_trigger()` calls it after its existing capability/nonce/product/AS-availability checks, unchanged behavior for the manual button
- [ ] New `STCRM_Backfill::sync_all_products(): void` — loops `STCRM_Settings::get_settings()['products']`, skips non-Freemius rows (`is_freemius_product()`) and rows with no decrypted `api_token`, skips any product whose `get_status()` is `'running'`, calls `trigger_for_product()` for the rest
- [ ] New Settings → Advanced field `freemius_sync_interval` — `<select>` of `hourly`/`twicedaily`/`daily`/`weekly`, default `daily`; save handler validates against the 4 allowed values, falling back to the default on anything else
- [ ] `handle_save()`'s `'advanced'` case: when `freemius_sync_interval` changes, `wp_clear_scheduled_hook( 'stcrm_recurring_freemius_sync' )` then `wp_schedule_event( time(), $new_interval, 'stcrm_recurring_freemius_sync' )` — reschedule takes effect immediately, first run fires right away
- [ ] New recurring hook `stcrm_recurring_freemius_sync` wired in `class-sublime-crm.php`'s `define_cron_hooks()` → a new thin `SublimeCRM::sync_all_freemius_products()` wrapper calling `STCRM_Backfill::sync_all_products()`, same pattern as the existing `stcrm_auto_close_tickets` entry
- [ ] `STCRM_Activator::schedule_events()` adds `stcrm_recurring_freemius_sync` to its scheduled-if-not-already-scheduled list, using the default interval (`daily`)
- [ ] `STCRM_Deactivator` + `uninstall.php` clear `stcrm_recurring_freemius_sync`, matching the existing `stcrm_auto_close_tickets`/`stcrm_purge_expired_tokens`/`stcrm_purge_old_logs` entries in both files
- [x] Extract `STCRM_Backfill::trigger_for_product( int $product_id ): void` from `handle_trigger()`'s reset+enqueue logic; `handle_trigger()` calls it after its existing capability/nonce/product/AS-availability checks, unchanged behavior for the manual button
- [x] New `STCRM_Backfill::sync_all_products(): void` — loops `STCRM_Settings::get_settings()['products']`, skips non-Freemius rows and rows with no decrypted `api_token`, skips any product whose `get_status()` is `'running'`, calls `trigger_for_product()` for the rest
- [x] New Settings → Advanced field `freemius_sync_interval` — `<select>` of `hourly`/`twicedaily`/`daily`/`weekly`, default `daily`; save handler validates against `STCRM_Settings::VALID_SYNC_INTERVALS`, falling back to the default on anything else
- [x] `handle_save()`'s `'advanced'` case reschedules immediately when `freemius_sync_interval` changes (`wp_unschedule_event()` + `wp_schedule_event( time(), ... )`) — first run fires right away
- [x] New recurring hook `stcrm_recurring_freemius_sync` wired in `class-sublime-crm.php`'s `define_cron_hooks()` → `SublimeCRM::sync_all_freemius_products()` → `STCRM_Backfill::sync_all_products()`
- [x] `STCRM_Activator::schedule_events()` schedules `stcrm_recurring_freemius_sync` at `daily` if not already scheduled
- [x] `STCRM_Deactivator` + `uninstall.php` clear `stcrm_recurring_freemius_sync`
- Files: `includes/Services/class-stcrm-backfill.php`, `admin/class-stcrm-settings.php`, `includes/class-sublime-crm.php`, `includes/class-stcrm-activator.php`, `includes/class-stcrm-deactivator.php`, `uninstall.php`

**Process:** built as one cohesive change (not split into sub-items), same cadence as Phase 10/11/12.

**Implementation notes (2026-07-13):**
- `wp_schedule_event()` only ships `hourly`/`twicedaily`/`daily` as WP-core built-in schedules — `weekly` isn't one of them, so a new `SublimeCRM::register_weekly_cron_schedule()` hooks `cron_schedules` (added to `define_cron_hooks()`) to add it (`WEEK_IN_SECONDS`), guarded so it only adds the key if not already present (defensive against another plugin registering the same name).
- `trigger_for_product()` is a straight extraction — same 3 option writes + `as_enqueue_async_action()` call `handle_trigger()` always ran, just callable from `sync_all_products()` too. `handle_trigger()`'s own capability/nonce/AS-availability/product-lookup checks are untouched, so the manual "Run Backfill" button's behavior is identical to before this phase.
- `sync_all_products()` deliberately does no HTTP/API work itself — it only resets options and enqueues the existing async `stcrm_run_backfill_page` job (same as the manual button), so the recurring cron tick itself stays fast regardless of how many products are configured; the actual Freemius API pagination happens in the already-existing AS job chain.
- `handle_save()`'s reschedule branch only fires when the posted value differs from the currently-saved one — saving the Advanced tab with an unchanged interval does not touch the schedule.

**Verified (2026-07-13):** a PHP-CLI script (13 assertions) against the real local install: `VALID_SYNC_INTERVALS` and the default `freemius_sync_interval` correct; `weekly` present in `wp_get_schedules()` with the correct `WEEK_IN_SECONDS` interval; `STCRM_Activator::activate()` (re-run, idempotent) schedules `stcrm_recurring_freemius_sync`; `trigger_for_product()` correctly resets a product's status/page/total options and enqueues a real `stcrm_run_backfill_page` Action Scheduler action (confirmed via `as_get_scheduled_actions()`, then unscheduled); `sync_all_products()` against 4 fake product rows correctly triggered only the idle Freemius product with a token, left an already-`running` product's progress untouched, and skipped both a no-token Freemius row and a `wporg` row. A second round drove the real Advanced-tab save path directly (`STCRM_Settings::handle_save()`, with a real admin user + real nonce, tab=`advanced`, `freemius_sync_interval=weekly`) in one PHP-CLI process, then read the result back in a fresh process (since `handle_save()` ends via its own `wp_safe_redirect()`+`exit()`): confirmed the setting persisted as `weekly`, `wp_get_schedule('stcrm_recurring_freemius_sync')` returned `weekly`, and a next-run timestamp existed. Restored the site back to `daily` afterward via the same real save path. `php -l` clean on all 6 touched files. All test option rows (fake product IDs 999001/999101–999104) and one-off verification scripts cleaned up afterward.

---

## PHASE 16 — Configurable Portal & Launcher Copy

> User asked (2026-07-14, 4 screenshots): why the Support Portal block has no editor settings at all (`editor.jsx` is a bare "renders on the frontend" placeholder), and how to update the text shown across the New Ticket form and its various states (cap-reached, magic-link-sent, etc.) — every string in every portal view (`NewTicketView.jsx`, `CapReachedView.jsx`, `AuthView.jsx`, `MyTicketsView.jsx`, `EmptyView.jsx`, `ExpiredView.jsx`, `ThreadView.jsx`) and in the floating launcher (`Launcher.jsx`) is hardcoded English, with zero admin-facing way to change any of it. Designed via `superpowers:brainstorming` (one question at a time, 2026-07-14).

**Design decisions (confirmed via brainstorming, 2026-07-14):**
- **Full coverage, not just the highest-value strings.** Every primary UI string — headings, descriptions, field labels/placeholders, button text, and static state screens — becomes customizable across all 7 portal views and the launcher. Client-side fallback/error messages (e.g. "Something went wrong. Please try again.") and anything server-driven (real 409/429 API error text) stay hardcoded — customizing those risks admins accidentally hiding real debugging signal, and almost no one would ever touch them.
- **Split storage by surface, matching each surface's own nature.** The portal is a content block — its text lives as a **block attribute** (`copy`, type `object`, sparse — only explicitly-customized keys, default `{}`), edited per-block-instance via Inspector Controls. The launcher is a site-wide singleton, already configured via global Settings — its text lives as a **new `launcher_copy` setting** (sparse array, default `[]`) in `stcrm_settings`, on a **new dedicated "Launcher" Settings tab** (the existing `launcher_enabled` toggle moves here too, out of Advanced — Advanced already has 6 unrelated ops/system fields and would get unreadable with ~15-20 more added).
- **Single source of truth for default English copy, one per surface.** New `src/portal/copy-defaults.js` and `src/launcher/copy-defaults.js` — flat key→string maps (~60-80 keys for the portal across its 7 views, ~15-20 for the launcher). Every view component stops hardcoding JSX text and instead reads from a merged `{ ...DEFAULTS, ...overrides }` object. Sparse-by-default storage means an untouched install renders byte-identical output to today — no migration needed, no visual change until an admin actually customizes something.
- **One accepted duplication, explicit trade-off:** the portal's Inspector Controls are React and can `import` `copy-defaults.js` directly for placeholder text — zero duplication. The launcher's new Settings tab is a plain server-rendered PHP form (matching every other tab in this plugin), so PHP can't import a JS module; showing "current default" as placeholder text there requires a small mirrored `STCRM_Launcher_Copy::DEFAULTS` PHP constant, written once alongside the JS one. Accepted as low-risk (static English strings, both files touched together in this one phase, worst-case drift is cosmetic only) in exchange for the better UX of real placeholders instead of a generic "leave blank for default" hint.
- **Portal block editor gets a state-switchable live-look preview, not a static placeholder.** Every one of the 7 view components (plus splitting `NewTicketView`'s embedded `SuccessState` and `AuthView`'s two sub-states out as their own previewable screens — 9 total) gains a `previewMode` boolean prop: when true, every API-calling `useEffect` (product fetch, session detection, thread polling) is skipped in favor of small fixed sample data, and every click/submit handler becomes a no-op (`preventDefault()` only). Markup/CSS stay byte-for-byte identical to the live component — the preview *is* the real component, inert, not a separately-maintained mockup that can drift out of sync. This was a **safety requirement, not just a nice-to-have**: the portal isn't a passive display, `NewTicketView`/`AuthView` make real mutating REST calls (create a ticket, send a real customer email) — rendering the *live* app inside the block editor risked an editor accidentally creating real tickets or emailing real customers while just previewing the block, plus the portal's own router manipulates `window.history` in a way that would collide with the editor's own `?post=N&action=edit` URL.
- **Editor UI:** a **state switcher** (dropdown, 9 options: New Ticket, Ticket Submitted, Cap Reached, Sign In, Check Your Inbox, Expired Link, My Tickets, Empty State, Thread) swaps which component renders in the canvas, in `previewMode`, fed live sample data. **Inspector Controls** are grouped into one `PanelBody` per screen (same 9-way grouping as the switcher) — selecting a state in the switcher auto-expands its matching panel and collapses the others, so editing never means scrolling through ~80 fields to find the ~8 relevant to what's currently on screen. Typing in a field updates the `copy` attribute immediately, live-updating the canvas preview.

- [x] `src/portal/copy-defaults.js` — flat default-copy map for all 9 portal screens (~60-80 keys), single source of truth
- [x] `src/launcher/copy-defaults.js` — flat default-copy map for the launcher's screens (~15-20 keys)
- [x] `STCRM_Launcher_Copy::DEFAULTS` (new small PHP class/constant) — mirrors `src/launcher/copy-defaults.js`, used only for Settings-tab placeholder text
- [x] `blocks/support-portal/block.json` — add `attributes: { copy: { type: 'object', default: {} } }`
- [x] `blocks/support-portal/render.php` — pass `$attributes['copy']` through `wp_localize_script`'s `stcrmPortal` object (raw, sparse — merge with defaults happens client-side)
- [x] Every portal view component (`NewTicketView.jsx` incl. its embedded `SuccessState`, `CapReachedView.jsx`, `AuthView.jsx`'s two sub-states, `MyTicketsView.jsx`, `EmptyView.jsx`, `ExpiredView.jsx`, `ThreadView.jsx`) — replace hardcoded JSX text with a merged-copy read (context or prop, avoiding prop-drilling through the view tree); add `previewMode` support (skip API-calling `useEffect`s in favor of fixed sample data, no-op handlers)
- [x] `src/portal/editor.jsx` — rewritten from the bare placeholder into a real component: state-switcher dropdown, per-screen `InspectorControls` `PanelBody`s (`TextControl`/`TextareaControl`, placeholder = `copy-defaults.js` value), canvas renders the selected view component in `previewMode` with live attribute values
- [x] `Launcher.jsx` — same merged-copy read pattern as the portal, reading from `window.stcrmLauncher.copy` (localized sparse overrides merged client-side with `src/launcher/copy-defaults.js`)
- [x] `includes/class-stcrm-launcher.php` — localize `launcher_copy` setting into `window.stcrmLauncher.copy`
- [x] `STCRM_Settings::$defaults` — add `launcher_copy => []`
- [x] New "Launcher" Settings tab — moves the existing `launcher_enabled` checkbox here from Advanced; adds grouped text fields for every launcher screen, placeholder text from `STCRM_Launcher_Copy::DEFAULTS`
- [x] `handle_save()` — new `'launcher'` tab case, sparse-saves only non-empty overridden fields into `launcher_copy`
- Files: `blocks/support-portal/block.json`, `blocks/support-portal/render.php`, `src/portal/editor.jsx`, `src/portal/copy-defaults.js` (new), `src/portal/NewTicketView.jsx`, `src/portal/CapReachedView.jsx`, `src/portal/AuthView.jsx`, `src/portal/MyTicketsView.jsx`, `src/portal/EmptyView.jsx`, `src/portal/ExpiredView.jsx`, `src/portal/ThreadView.jsx`, `src/launcher/Launcher.jsx`, `src/launcher/copy-defaults.js` (new), `includes/class-stcrm-launcher.php`, `admin/class-stcrm-settings.php`, `includes/class-stcrm-launcher-copy.php` (new, PHP defaults mirror)

**Process:** implementation plan written via `superpowers:writing-plans` (`plans/2026-07-14-phase-16-configurable-copy.md`, 16 tasks) and executed via `superpowers:subagent-driven-development` — fresh implementer subagent per task, dedicated task-scoped reviewer per task, controller-independent re-verification of every commit's hygiene (scope, no Co-Authored-By trailer, rebuilt-bundle-in-same-commit) before each review dispatch.

**Implementation notes (2026-07-14):**
- Task 1 (`copy-defaults.js`) round 1 shipped 14 straight-ASCII-apostrophe contractions inside single-quoted JS strings, silently breaking parse (`SyntaxError`) — self-review had only checked curly-quote correctness by eye. Fixed; round 2 review independently re-verified via the project's actual JSX-aware linter (`wp-scripts lint-js`), zero parse errors, all contractions confirmed curly.
- Task 5 (`AuthView.jsx`) review found a real, verified regression: the extracted `SentState`'s "use a different email" link called `navigate(VIEWS.AUTH)`, a no-op when already on `?view=auth` (no `key` on `<AuthView/>` in `App.jsx`'s `RouteView`, so React reconciles as an update not a remount) — both the real magic-link-sent screen and the honeypot's fake-sent path got permanently stuck. Fixed with an optional `onUseAnotherEmail` prop (default `navigate(VIEWS.AUTH)` for Task 11's zero-props preview use, explicit `() => setSent(false)` for the real flow).
- Task 7 (`MyTicketsView.jsx`) review flagged an Important finding traceable to the plan's own example code: a load-bearing comment explaining why sign-out must be a hard navigation (guards a real 2026-07-10 regression) was silently dropped. Restored; the same trap in Task 8's brief was flagged proactively in that task's dispatch and avoided on the first pass.
- Task 11 (`editor.jsx`) — the most architecturally significant task — dispatched its review on Opus given the safety-critical nature (the guarantee that the block-editor preview canvas never fires a real `/stcrm/v1/*` request). Both the controller's own live-browser re-verification attempt and, separately, Task 16's later live pass hit then resolved a persistent local-environment cookie-auth quirk (see Task 16 note below); Task 11's review fell back to rigorous static tracing of every `SCREENS` entry's `previewMode`/display-only status, confirmed clean, additionally noting the gate covers keyboard-triggered submission too, not just clicks.
- Task 12 (launcher `copy-defaults.js` + `Launcher.jsx`) implementer caught a real plan inaccuracy: the brief's field-mapping assumed the launcher's compose form mirrors the portal's (with Name/License Key fields) — it doesn't, it's the documented compact form (Email/Product/Subject/Message only per CLAUDE.md §13). Correctly skipped fabricating fields for those two sub-mappings rather than inventing new form UI.
- Task 14 (`STCRM_Launcher_Copy::DEFAULTS`) round 1 review did a full programmatic byte-diff (not a spot-check) of all 36 leaf strings against the JS source of truth and found one real drift: `sent.heading` used a straight apostrophe where JS uses curly — the implementer's own first self-review had falsely claimed an exact match. Fixed with a proper byte-level re-verification; round 2 independently re-derived both files from scratch and confirmed byte-identical.
- Task 15 (new Launcher Settings tab) round 1 review reproduced a real, live data-loss bug: moving the `launcher_enabled` checkbox's markup off the Advanced tab left its save-assignment orphaned in `handle_save()`'s `'advanced'` case — since the Advanced form no longer submits that field, saving Advanced for *any* unrelated reason (e.g. toggling Debug Logging) silently reset `launcher_enabled` to `0`. Fixed by deleting the orphaned line (sole write site is now the `'launcher'` case); round 2 independently reproduced the original bug scenario live and confirmed it no longer occurs.
- Task 16 (final integration verification) cracked the wp-admin cookie-auth quirk that blocked the controller's own live re-verification during Task 11: a bare PHP-CLI cookie-minting script has no `$_SERVER['HTTP_HOST']`, so `site_url()` resolves incorrectly, which makes `wp_cookie_constants()` compute the wrong `AUTH_COOKIE`/`LOGGED_IN_COOKIE` *name* hash (`md5(site_url())`) — the cookie's cryptographic contents were always valid, only the name the browser needed to send was wrong. Fixed by setting `$_SERVER['HTTP_HOST']` (and related fields) to match the real site before `wp-load.php`.

**Verified (2026-07-14):** every one of the 16 tasks individually verified and reviewed (13 approved on first review pass, 3 required one fix-and-re-review round — Tasks 5, 14, 15 above); final Task 16 integration pass confirmed, against the real local site with real REST/DB round trips (no mocks): (1) an untouched install renders byte-identical to pre-Phase-16 across both the portal and launcher; (2) a real block-attribute override (portal) and a real `launcher_copy` Settings override (launcher) apply simultaneously with zero cross-interference, sparse-override behavior confirmed (an untouched sibling field correctly stays at its default, not overwritten); (3) the block editor's 9-screen preview switcher works end-to-end — zero `/stcrm/v1/*` requests and zero JS errors across the full pass, including active clicking inside the inert canvas at every state; (4) final state confirmed clean — no leftover overrides, test tickets/contacts, session tokens, or stray files, independently re-confirmed by the controller via direct DB/settings reads and a live Playwright spot-check. `php -l` clean on every touched PHP file; `npm run build` clean throughout, rebuilt bundle shipped in the same commit as every JS source change. Full 16-task plan + task-by-task implementation notes in `plans/2026-07-14-phase-16-configurable-copy.md`.
...
| 12 — Free (WP.org) Product Support | — | 1 group — ✅ COMPLETE (2026-07-08) | 8 tasks |
| 13 — WP Account Login for Portal | — | 1 group — ✅ COMPLETE (2026-07-08) | 11 tasks |
| 14 — Global Contact Identity Model | — | 1 group — ✅ COMPLETE (2026-07-13) | 14 tasks + 4 ad-hoc follow-ups |
| 15 — Automatic Freemius Entitlement Sync | — | 1 group — ✅ COMPLETE (2026-07-13) | 7 tasks |
| 16 — Configurable Portal & Launcher Copy | — | 1 group — ✅ COMPLETE (2026-07-14) | 13 tasks |
| **Total** | **10 weeks + gap closure** | **65 groups** | **~280 tasks** |
